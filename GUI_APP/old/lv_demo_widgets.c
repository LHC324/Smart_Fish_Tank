 ...
 *  STATIC PROTOTYPES
 **********************/
static void controls_create(lv_obj_t * parent); //创建第一个界面
static void visuals_create(lv_obj_t * parent);//创建第二个界面
static void selectors_create(lv_obj_t * parent);//创建第三个界面
static  void toning_create(lv_obj_t * parent);//创建第四个界面
...
{
    tv = lv_tabview_create(lv_scr_act(), NULL); //创建一个标签视图对象
#if LV_USE_THEME_MATERIAL //使用主题材料
    ...
    t1 = lv_tabview_add_tab(tv, "Con");//标题1Controls
    t2 = lv_tabview_add_tab(tv, "Vis");//标题2Visuals
    t3 = lv_tabview_add_tab(tv, "Sel");//标题3Selectors
    t4 = lv_tabview_add_tab(tv, "Ton");//标题4Toning

    lv_style_init(&style_box); //初始化样式
    lv_style_set_value_align(&style_box, LV_STATE_DEFAULT, LV_ALIGN_OUT_TOP_LEFT);//状态默认,左对齐
...
static void controls_create(lv_obj_t * parent) //创建第一个主界面
{
    lv_page_set_scrl_layout(parent, LV_LAYOUT_PRETTY_TOP); //行顶部对齐

    lv_disp_size_t disp_size = lv_disp_get_size_category(NULL);
    lv_coord_t grid_w = lv_page_get_width_grid(parent, disp_size <= LV_DISP_SIZE_SMALL ? 1 : 2, 1);

#if LV_DEMO_WIDGETS_SLIDESHOW == 0 //演示小部件幻灯片
    static const char * btns[] = {"Cancel", "Ok", ""}; //创建一个消息弹出窗口

    lv_obj_t * m = lv_msgbox_create(lv_scr_act(), NULL);//第一参数为屏幕调用，第二参数为需要复制的子类型
    lv_msgbox_add_btns(m, btns); //创建按钮矩阵
    lv_obj_t * btnm = lv_msgbox_get_btnmatrix(m); //获取消息框按钮矩阵
    lv_btnmatrix_set_btn_ctrl(btnm, 1, LV_BTNMATRIX_CTRL_CHECK_STATE); //设置(清除)按钮的控制属性
#endif //LV_BTNMATRIX_CTRL_CHECK_STATE:当前已切换按钮（例如选中）

    lv_obj_t * h = lv_cont_create(parent, NULL);
    lv_cont_set_layout(h, LV_LAYOUT_PRETTY_MID); // 在容器上设置布局
    lv_obj_add_style(h, LV_CONT_PART_MAIN, &style_box); // 向对象使用添加样式//LV_CONT_PART_MAIN：主要零件
    lv_obj_set_drag_parent(h, true); //启用使用父级进行拖动相关操作

    lv_obj_set_style_local_value_str(h, LV_CONT_PART_MAIN, LV_STATE_DEFAULT, "Basics"); //设置第一页上一级子部件

    lv_cont_set_fit2(h, LV_FIT_NONE, LV_FIT_TIGHT); //使用不同的自动设置水平和垂直
    lv_obj_set_width(h, grid_w); //对象大小：单轴修改
    ...
    lv_btn_toggle(btn); //切换按钮的状态
    label = lv_label_create(btn, NULL);
    lv_label_set_text(label ,"Button");

    lv_switch_create(h, NULL); //创建一个开关对象

    lv_checkbox_create(h, NULL); //创建复选框对象

    lv_coord_t fit_w = lv_obj_get_width_fit(h); // 用左、右填充缩小宽度

    lv_obj_t * slider = lv_slider_create(h, NULL); //创建一个滑块对象
    lv_slider_set_value(slider, 0, LV_ANIM_OFF); //LV_ANIM_OFF:某种情况禁用动画
    lv_obj_set_event_cb(slider, slider_event_cb);
    lv_obj_set_width_margin(slider, fit_w); // 以设置宽度或高度减少的边缘

    /*Use the knobs style value the display the current value in focused state使用旋钮样式值在聚焦状态下显示当前值*/
    ...
    lv_slider_set_type(slider, LV_SLIDER_TYPE_RANGE); //LV_SLIDER_TYPE_RANGE:滑块类型范围
...
static void visuals_create(lv_obj_t * parent) //创建第二个主界面
{
...
static void selectors_create(lv_obj_t * parent) //创建第三个主界面
{
...
//} invalid storage class for function:划花括号不匹配

static  void toning_create(lv_obj_t * parent)//创建第四个主界面
{
        lv_page_set_scrl_layout(parent, LV_LAYOUT_PRETTY_MID);//继承父类，初始化布局
        lv_disp_size_t disp_size = lv_disp_get_size_category(NULL); //配合lv_cont_set_fit2把调色盘放入框中
        lv_coord_t grid_w = lv_page_get_width_grid(parent, disp_size <= LV_DISP_SIZE_SMALL ? 1 : 2, 1);
        ...
        lv_obj_add_style(h, LV_CONT_PART_MAIN, &style_box); // 向对象使用添加样式//LV_CONT_PART_MAIN：主要零件
        lv_obj_set_drag_parent(h, true); //启用使用父级进行拖动相关操作
        lv_obj_set_style_local_value_str(h, LV_CONT_PART_MAIN, LV_STATE_DEFAULT, "Cpicker"); //设置第一页上一级子部件
        lv_cont_set_fit2(h, LV_FIT_NONE, LV_FIT_TIGHT); //使用不同的自动设置水平和垂直
        lv_obj_set_width(h, grid_w); //对象大小：单轴修改

        lv_obj_t * cpicker;
        cpicker = lv_cpicker_create(h, NULL);
        lv_obj_set_size(cpicker, 143, 143);
        //lv_obj_align(cpicker, NULL, LV_ALIGN_CENTER, 0, 0);//居中对齐
}

static void slider_event_cb(lv_obj_t * slider, lv_event_t e)
{
    if(e == LV_EVENT_VALUE_CHANGED) { //事件值已更改
        if(lv_slider_get_type(slider) == LV_SLIDER_TYPE_NORMAL) { //滑轨类型正常
            static char buf[16];
