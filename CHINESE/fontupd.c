...
{	
	...
	if(rval==0)//字库文件都存在.
	{  
		LCD_ShowString(x,y,128,12,size,"sectors...");//提示正在擦除扇区	
		for(i=0;i<FONTSECSIZE;i++)	//先擦除字库区域,提高写入速度
		{
			fupd_prog(x+20*size/4,y,size,FONTSECSIZE,i);//进度显示
