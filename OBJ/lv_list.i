#line 1 "..\\GUI\\lvgl_src\\lv_widgets\\lv_list.c"
...
#line 10 "..\\GUI\\lvgl_src\\lv_widgets\\lv_list.c"


#line 13 "..\\GUI\\lvgl_src\\lv_widgets\\lv_list.c"
#line 14 "..\\GUI\\lvgl_src\\lv_widgets\\lv_list.c"
...
#line 15 "..\\GUI\\lvgl_src\\lv_widgets\\lv_list.c"
#line 16 "..\\GUI\\lvgl_src\\lv_widgets\\lv_list.c"
#line 17 "..\\GUI\\lvgl_src\\lv_widgets\\lv_list.c"
...
static _Bool lv_list_is_list_btn(lv_obj_t * list_btn);
static _Bool lv_list_is_list_img(lv_obj_t * list_btn);
static _Bool lv_list_is_list_label(lv_obj_t * list_btn);
...
lv_obj_t * lv_list_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    do { if(!((lv_debug_check_null(list)))) { ; lv_debug_log_error("Out of memory", (uint64_t)((uintptr_t)list)); while(1); } } while(0);;
    if(list == 0) return 0;

    if(ancestor_page_signal == 0) ancestor_page_signal = lv_obj_get_signal_cb(list);
    ...
    if(copy == 0) {
        lv_page_set_anim_time(list, 100);
        lv_page_set_scrollable_fit2(list, LV_FIT_PARENT, LV_FIT_TIGHT);
        lv_obj_set_size(list, 2 * 60, 3 * 60);
        ...
        lv_obj_t * copy_btn = lv_list_get_next_btn(copy, 0);
        while(copy_btn) {
            const void * img_src = 0;
        ...
        lv_obj_refresh_style(list, LV_OBJ_PART_ALL, 0xFF);
    }
...
void lv_list_clean(lv_obj_t * list)
{
    do { if(!((lv_debug_check_null(list)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)list)); while(1); } } while(0);;
...
lv_obj_t * lv_list_add_btn(lv_obj_t * list, const void * img_src, const char * txt)
{
    ...
    lv_obj_t * last_btn = lv_list_get_prev_btn(list, 0);
    ...
    lv_obj_t * btn;
    btn = lv_btn_create(list, 0);

    lv_obj_add_protect(btn, LV_PROTECT_CHILD_CHG);

     
    if(ancestor_btn_signal == 0) ancestor_btn_signal = lv_obj_get_signal_cb(btn);
    ...
    if(layout_ver) {
        ...
        lv_coord_t w = last_btn ? lv_obj_get_width(last_btn) : (60 * 3) / 2;
    ...
    lv_obj_t * img = 0;
    if(img_src) {
        img = lv_img_create(btn, 0);
        lv_img_set_src(img, img_src);
        lv_obj_set_click(img, 0);
        if(img_signal == 0) img_signal = lv_obj_get_signal_cb(img);
    }

    if(txt != 0) {
        lv_obj_t * label = lv_label_create(btn, 0);
        lv_label_set_text(label, txt);
        lv_obj_set_click(label, 0);
        ...
        if(label_signal == 0) label_signal = lv_obj_get_signal_cb(label);
    }
 ...
 
    {
        ...
        if(lv_list_get_next_btn(list, 0) == btn && lv_group_get_focused(g) == list) {
    ...
    btn->signal_cb(btn, LV_SIGNAL_CHILD_CHG, 0);
...
_Bool lv_list_remove(const lv_obj_t * list, uint16_t index)
{
    ...
    uint16_t count = 0;
    lv_obj_t * e   = lv_list_get_next_btn(list, 0);
    while(e != 0) {
        if(count == index) {
            lv_obj_del(e);
            return 1;
...
void lv_list_focus_btn(lv_obj_t * list, lv_obj_t * btn)
{
    do { if(!((lv_debug_check_null(list)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)list)); while(1); } } while(0);;
    if(btn) {
        do { if(!((lv_debug_check_null(btn)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)btn)); while(1); } } while(0);;
    }
...
void lv_list_set_layout(lv_obj_t * list, lv_layout_t layout)
{
    ...
    lv_obj_t * btn = lv_list_get_prev_btn(list, 0);
...
lv_obj_t * lv_list_get_btn_label(const lv_obj_t * btn)
{
    ...
    lv_obj_t * label = lv_obj_get_child_back(btn, 0);
    if(label == 0) return 0;

    while(lv_list_is_list_label(label) == 0) {
...
lv_obj_t * lv_list_get_btn_img(const lv_obj_t * btn)
{
    ...
    lv_obj_t * img = lv_obj_get_child_back(btn, 0);
    if(img == 0) return 0;

    while(lv_list_is_list_img(img) == 0) {
...
lv_obj_t * lv_list_get_prev_btn(const lv_obj_t * list, lv_obj_t * prev_btn)
{
    ...
    while(lv_list_is_list_btn(btn) == 0) {
...
uint16_t lv_list_get_size(const lv_obj_t * list)
{
    ...
    uint16_t size = 0;
    lv_obj_t * btn = lv_list_get_next_btn(list, 0);
    while(btn) {
        size++;
...
static lv_res_t lv_list_signal(lv_obj_t * list, lv_signal_t sign, void * param)
{
    ...
    if(sign == LV_SIGNAL_GET_TYPE) return lv_obj_handle_get_type_signal(param, "lv_list");
    ...
    if(sign == LV_SIGNAL_RELEASED || sign == LV_SIGNAL_PRESSED || sign == LV_SIGNAL_PRESSING ||
       sign == LV_SIGNAL_LONG_PRESS || sign == LV_SIGNAL_LONG_PRESS_REP) {
           ...
           (indev_type == LV_INDEV_TYPE_ENCODER && lv_group_get_editing(lv_obj_get_group(list)))) {
            ...
            if(ext->act_sel_btn) {
                ...
                if(sign == LV_SIGNAL_PRESSED) {
                    res = lv_event_send(ext->act_sel_btn, LV_EVENT_PRESSED, 0);
                    if(res != LV_RES_OK) return res;
                }
                else if(sign == LV_SIGNAL_PRESSING) {
                    res = lv_event_send(ext->act_sel_btn, LV_EVENT_PRESSING, 0);
                    if(res != LV_RES_OK) return res;
                }
                else if(sign == LV_SIGNAL_LONG_PRESS) {
                    res = lv_event_send(ext->act_sel_btn, LV_EVENT_LONG_PRESSED, 0);
                    if(res != LV_RES_OK) return res;
                }
                else if(sign == LV_SIGNAL_LONG_PRESS_REP) {
                    res = lv_event_send(ext->act_sel_btn, LV_EVENT_LONG_PRESSED_REPEAT, 0);
                    if(res != LV_RES_OK) return res;
                }
                else if(sign == LV_SIGNAL_RELEASED) {
                    if(indev->proc.long_pr_sent == 0) {
                        res = lv_event_send(ext->act_sel_btn, LV_EVENT_SHORT_CLICKED, 0);
                        if(res != LV_RES_OK) return res;
                    }
                    if(lv_indev_is_dragging(indev) == 0) {
                        res = lv_event_send(ext->act_sel_btn, LV_EVENT_CLICKED, 0);
                        if(res != LV_RES_OK) return res;
                    }

                    res = lv_event_send(ext->act_sel_btn, LV_EVENT_RELEASED, 0);
                    if(res != LV_RES_OK) return res;
                }
            }
        }
        ...
        if((indev_type == LV_INDEV_TYPE_KEYPAD) ||
           (indev_type == LV_INDEV_TYPE_ENCODER && lv_group_get_editing(g))) {
            ...
            else lv_list_focus_btn(list, lv_list_get_next_btn(list, 0));
        }
        if(indev_type == LV_INDEV_TYPE_ENCODER && lv_group_get_editing(g) == 0) {
            lv_list_ext_t * ext = lv_obj_get_ext_attr(list);
        ...
        *editable       = 1;
        ...
        if(c == LV_KEY_RIGHT || c == LV_KEY_DOWN) {
            ...
            res = lv_event_send(ext->act_sel_btn, LV_EVENT_CANCEL, 0);
        }
...
static lv_res_t lv_list_btn_signal(lv_obj_t * btn, lv_signal_t sign, void * param)
{
    ...
    if((sign == LV_SIGNAL_COORD_CHG && (lv_obj_get_width(btn) != lv_area_get_width(param) ||
       ...
       sign == LV_SIGNAL_STYLE_CHG) {
        ...
        if(ext->last_sel_btn == btn) ext->last_sel_btn = 0;
...
static _Bool lv_list_is_list_btn(lv_obj_t * list_btn)
{
    ...
    uint8_t cnt;
    for(cnt = 0; cnt < 8; cnt++) {
        if(type.type[cnt] == 0) break;
        if(!strcmp(type.type[cnt], "lv_btn")) return 1;
    }
    return 0;
}
...
static _Bool lv_list_is_list_label(lv_obj_t * list_label)
{
    lv_obj_type_t type;
    ...
    for(cnt = 0; cnt < 8; cnt++) {
        if(type.type[cnt] == 0) break;
        if(!strcmp(type.type[cnt], "lv_label")) return 1;
    }
    return 0;
}
...
static _Bool lv_list_is_list_img(lv_obj_t * list_img)
{
    lv_obj_type_t type;
    ...
    for(cnt = 0; cnt < 8; cnt++) {
        if(type.type[cnt] == 0) break;
        if(!strcmp(type.type[cnt], "lv_img")) return 1;
    }
    return 0;
}
