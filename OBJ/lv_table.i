#line 1 "..\\GUI\\lvgl_src\\lv_widgets\\lv_table.c"
...
#line 10 "..\\GUI\\lvgl_src\\lv_widgets\\lv_table.c"


#line 13 "..\\GUI\\lvgl_src\\lv_widgets\\lv_table.c"
...
#line 14 "..\\GUI\\lvgl_src\\lv_widgets\\lv_table.c"
#line 15 "..\\GUI\\lvgl_src\\lv_widgets\\lv_table.c"
#line 16 "..\\GUI\\lvgl_src\\lv_widgets\\lv_table.c"
#line 17 "..\\GUI\\lvgl_src\\lv_widgets\\lv_table.c"
...
#line 18 "..\\GUI\\lvgl_src\\lv_widgets\\lv_table.c"
...
#line 19 "..\\GUI\\lvgl_src\\lv_widgets\\lv_table.c"
...
lv_obj_t * lv_table_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    do { if(!((lv_debug_check_null(table)))) { ; lv_debug_log_error("Out of memory", (uint64_t)((uintptr_t)table)); while(1); } } while(0);;
    if(table == 0) return 0;
    ...
    if(ancestor_signal == 0) ancestor_signal = lv_obj_get_signal_cb(table);
    if(ancestor_design == 0) ancestor_design = lv_obj_get_design_cb(table);
    ...
    if(copy == 0) {
        ...
        lv_obj_refresh_style(table, LV_OBJ_PART_ALL, 0xFF);
    }
...
void lv_table_set_cell_value(lv_obj_t * table, uint16_t row, uint16_t col, const char * txt)
{
    do { if(!((lv_debug_check_null(table)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)table)); while(1); } } while(0);;
...
#line 182 "..\\GUI\\lvgl_src\\lv_widgets\\lv_table.c"
    ext->cell_data[cell] = lv_mem_realloc(ext->cell_data[cell], strlen(txt) + 2);  
	do { if(!((lv_debug_check_null(ext->cell_data[cell])))) { ; lv_debug_log_error("Out of memory", (uint64_t)((uintptr_t)ext->cell_data[cell])); while(1); } } while(0);;
	if(ext->cell_data[cell] == 0) return;
...
#line 271 "..\\GUI\\lvgl_src\\lv_widgets\\lv_table.c"
    ext->cell_data[cell] = lv_mem_realloc(ext->cell_data[cell], len + 2);  
    do { if(!((lv_debug_check_null(ext->cell_data[cell])))) { ; lv_debug_log_error("Out of memory", (uint64_t)((uintptr_t)ext->cell_data[cell])); while(1); } } while(0);;
    if(ext->cell_data[cell] == 0) {
        __va_end(ap2);
        return;
    }

    ext->cell_data[cell][len + 1] = 0;  
...
void lv_table_set_row_cnt(lv_obj_t * table, uint16_t row_cnt)
{
    ...
    if(ext->row_cnt > 0) {
        ...
        do { if(!((lv_debug_check_null(ext->row_h)))) { ; lv_debug_log_error("Out of memory", (uint64_t)((uintptr_t)ext->row_h)); while(1); } } while(0);;
        if(ext->row_h == 0) return;
    }
    else {
    ...
    if(ext->row_cnt > 0 && ext->col_cnt > 0) {
        ...
        do { if(!((lv_debug_check_null(ext->cell_data)))) { ; lv_debug_log_error("Out of memory", (uint64_t)((uintptr_t)ext->cell_data)); while(1); } } while(0);;
        if(ext->cell_data == 0) return;
...
_Bool lv_table_get_cell_merge_right(lv_obj_t * table, uint16_t row, uint16_t col)
{
    ...
    if(ext->cell_data[cell] == 0)
        ...
        lv_table_cell_format_t format;
        format.format_byte = ext->cell_data[cell][0];
        return format.s.right_merge ? 1 : 0;
    }
}
...
static lv_design_res_t lv_table_design(lv_obj_t * table, const lv_area_t * clip_area, lv_design_mode_t mode)
{
    ...
    if(mode == LV_DESIGN_COVER_CHK) {
        ...
        lv_draw_rect_dsc_t rect_dsc[4];
        lv_draw_label_dsc_t label_dsc[4];
        lv_draw_line_dsc_t line_dsc[4];
        lv_style_int_t cell_left[4];
        lv_style_int_t cell_right[4];
        lv_style_int_t cell_top[4];
        lv_style_int_t cell_bottom[4];

        uint16_t i;
        for(i = 0; i < 4; i++) {
            if((ext->cell_types & (1 << i)) == 0) continue;  
        ...
        _Bool rtl = lv_obj_get_base_dir(table) == LV_BIDI_DIR_RTL ? 1 : 0;
        ...
        for(row = 0; row < ext->row_cnt; row++) {
            ...
            for(col = 0; col < ext->col_cnt; col++) {
                ...
                for(col_merge = 0; col_merge + col < ext->col_cnt - 1; col_merge++) {
                    if(ext->cell_data[cell + col_merge] != 0) {
                ...
                if(ext->cell_data[cell]) {
                    ...
                    if(label_mask_ok) {
                        lv_draw_label(&txt_area, &label_mask, &label_dsc[cell_type], ext->cell_data[cell] + 1, 0);
                    }
...
static lv_res_t lv_table_signal(lv_obj_t * table, lv_signal_t sign, void * param)
{
    ...
    if(sign == LV_SIGNAL_GET_TYPE) return lv_obj_handle_get_type_signal(param, "lv_table");
...
static void refr_size(lv_obj_t * table)
{
    ...
    lv_style_int_t cell_top[4];
    lv_style_int_t cell_bottom[4];
    lv_style_int_t letter_space[4];
    lv_style_int_t line_space[4];
    const lv_font_t * font[4];
                                 ...
                                 lv_style_int_t * cell_left, lv_style_int_t * cell_right, lv_style_int_t * cell_top, lv_style_int_t * cell_bottom)
{
    ...
    for(cell = row_start, col = 0; cell < row_start + ext->col_cnt; cell++, col++) {
        if(ext->cell_data[cell] != 0) {
            ...
            if(format.s.crop) {
                h_max = ((lv_font_get_line_height(font[cell_type]) + cell_top[cell_type] + cell_bottom[cell_type]) > (h_max) ? (lv_font_get_line_height(font[cell_type]) + cell_top[cell_type] + cell_bottom[cell_type]) : (h_max));
                ...
                h_max = ((txt_size . y + cell_top[cell_type] + cell_bottom[cell_type]) > (h_max) ? (txt_size . y + cell_top[cell_type] + cell_bottom[cell_type]) : (h_max));
                cell += col_merge;
                col += col_merge;
            }
        }
