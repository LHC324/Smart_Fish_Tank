#line 1 "..\\GUI\\lvgl_src\\lv_widgets\\lv_arc.c"
...
#line 10 "..\\GUI\\lvgl_src\\lv_widgets\\lv_arc.c"


#line 1 "..\\GUI\\lvgl_src\\lv_widgets\\../lv_core/lv_group.h"
...
#line 18 "..\\GUI\\lvgl_src\\lv_widgets\\../lv_core/lv_group.h"
...
#line 202 "..\\GUI\\lvgl_src\\lv_widgets\\../lv_core/lv_group.h"
...
#line 13 "..\\GUI\\lvgl_src\\lv_widgets\\lv_arc.c"
#line 1 "..\\GUI\\lvgl_src\\lv_widgets\\../lv_core/lv_indev.h"
...
#line 17 "..\\GUI\\lvgl_src\\lv_widgets\\../lv_core/lv_indev.h"
#line 18 "..\\GUI\\lvgl_src\\lv_widgets\\../lv_core/lv_indev.h"
#line 19 "..\\GUI\\lvgl_src\\lv_widgets\\../lv_core/lv_indev.h"
...
#line 14 "..\\GUI\\lvgl_src\\lv_widgets\\lv_arc.c"
#line 15 "..\\GUI\\lvgl_src\\lv_widgets\\lv_arc.c"
#line 16 "..\\GUI\\lvgl_src\\lv_widgets\\lv_arc.c"
...
#line 17 "..\\GUI\\lvgl_src\\lv_widgets\\lv_arc.c"
...
#line 18 "..\\GUI\\lvgl_src\\lv_widgets\\lv_arc.c"
...
lv_obj_t * lv_arc_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    do { if(!((lv_debug_check_null(arc)))) { ; lv_debug_log_error("Out of memory", (uint64_t)((uintptr_t)arc)); while(1); } } while(0);;
    if(arc == 0) return 0;
    ...
    if(ancestor_signal == 0) ancestor_signal = lv_obj_get_signal_cb(arc);
    if(ancestor_design == 0) ancestor_design = lv_obj_get_design_cb(arc);
    ...
    if(copy == 0) {
        lv_obj_set_click(arc, 1);
        lv_obj_add_protect(arc, LV_PROTECT_PRESS_LOST);
        lv_obj_set_ext_click_area(arc, 60 / 10, 60 / 10, 60 / 10, 60 / 10);
        ...
        lv_obj_refresh_style(arc, LV_OBJ_PART_ALL, 0xFF);
    }
...
void lv_arc_set_start_angle(lv_obj_t * arc, uint16_t start)
{
    do { if(!((lv_debug_check_null(arc)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)arc)); while(1); } } while(0);;
    ...
    if(((start - ext->arc_angle_start) > 0 ? (start - ext->arc_angle_start) : (-(start - ext->arc_angle_start))) >= 180) {
        lv_obj_invalidate(arc);
    }
     
    else if(ext->arc_angle_start > ext->arc_angle_end && start > ext->arc_angle_end) {
        inv_arc_area(arc, ((ext->arc_angle_start) < (start) ? (ext->arc_angle_start) : (start)), ((ext->arc_angle_start) > (start) ? (ext->arc_angle_start) : (start)),
                     LV_ARC_PART_INDIC);
    }
...
void lv_arc_set_end_angle(lv_obj_t * arc, uint16_t end)
{
    ...
    if(((end - ext->arc_angle_end) > 0 ? (end - ext->arc_angle_end) : (-(end - ext->arc_angle_end))) >= 180) {
        lv_obj_invalidate(arc);
    }
     
    else if(ext->arc_angle_end > ext->arc_angle_start && end > ext->arc_angle_start) {
        inv_arc_area(arc, ((ext->arc_angle_end) < (end) ? (ext->arc_angle_end) : (end)), ((ext->arc_angle_end) > (end) ? (ext->arc_angle_end) : (end)), LV_ARC_PART_INDIC);
    }
...
void lv_arc_set_bg_start_angle(lv_obj_t * arc, uint16_t start)
{
    ...
    if(((start - ext->bg_angle_start) > 0 ? (start - ext->bg_angle_start) : (-(start - ext->bg_angle_start))) >= 180) {
        lv_obj_invalidate(arc);
    }
     
    else if(ext->bg_angle_start > ext->bg_angle_end && start > ext->bg_angle_end) {
        inv_arc_area(arc, ((ext->bg_angle_start) < (start) ? (ext->bg_angle_start) : (start)), ((ext->bg_angle_start) > (start) ? (ext->bg_angle_start) : (start)), LV_ARC_PART_BG);
    }
...
void lv_arc_set_bg_end_angle(lv_obj_t * arc, uint16_t end)
{
    ...
    if(((end - ext->bg_angle_end) > 0 ? (end - ext->bg_angle_end) : (-(end - ext->bg_angle_end))) >= 180) {
        lv_obj_invalidate(arc);
    }
     
    else if(ext->bg_angle_end > ext->bg_angle_start && end > ext->bg_angle_start) {
        inv_arc_area(arc, ((ext->bg_angle_end) < (end) ? (ext->bg_angle_end) : (end)), ((ext->bg_angle_end) > (end) ? (ext->bg_angle_end) : (end)), LV_ARC_PART_BG);
    }
...
static lv_design_res_t lv_arc_design(lv_obj_t * arc, const lv_area_t * clip_area, lv_design_mode_t mode)
{
    ...
    if(mode == LV_DESIGN_COVER_CHK) {
        ...
        lv_coord_t indic_r = arc_r - (((((left_indic) > (right_indic) ? (left_indic) : (right_indic))) > (((top_indic) > (bottom_indic) ? (top_indic) : (bottom_indic))) ? (((left_indic) > (right_indic) ? (left_indic) : (right_indic))) : (((top_indic) > (bottom_indic) ? (top_indic) : (bottom_indic)))));
...
static lv_res_t lv_arc_signal(lv_obj_t * arc, lv_signal_t sign, void * param)
{
    ...
    if(sign == LV_SIGNAL_GET_TYPE) return lv_obj_handle_get_type_signal(param, "lv_arc");
    ...
    if(sign == LV_SIGNAL_PRESSING) {
        ...
        angle = 360 - _lv_atan2(p.x, p.y) + 90;  
        ...
        angle = ext->last_angle + delta_angle;  
        ...
        if(new_value != lv_arc_get_value(arc)) {
            ...
            if(new_value != old_value) {
                res = lv_event_send(arc, LV_EVENT_VALUE_CHANGED, 0);
                if(res != LV_RES_OK) return res;
            }
        }
...
static void inv_arc_area(lv_obj_t * arc, uint16_t start_angle, uint16_t end_angle, lv_arc_part_t part)
{
    ...
    if(lv_obj_is_visible(arc) == 0) return;
    ...
    lv_coord_t rout       = (((lv_obj_get_width(arc) - left - right) < (lv_obj_get_height(arc) - top - bottom) ? (lv_obj_get_width(arc) - left - right) : (lv_obj_get_height(arc) - top - bottom))) / 2;
    ...
    if(part == LV_ARC_PART_INDIC && lv_style_list_get_style(&ext->style_knob, 0) != 0) {
        ...
        knob_extra_size += (((((knob_left) > (knob_right) ? (knob_left) : (knob_right))) > (((knob_top) > (knob_bottom) ? (knob_top) : (knob_bottom))) ? (((knob_left) > (knob_right) ? (knob_left) : (knob_right))) : (((knob_top) > (knob_bottom) ? (knob_top) : (knob_bottom)))));

        extra_area = ((extra_area) > (w / 2 + 2 + knob_extra_size) ? (extra_area) : (w / 2 + 2 + knob_extra_size));
    ...
    if(start_quarter == end_quarter && start_angle <= end_angle) {
        if(start_quarter == 0) {
            inv_area.y1 = y + ((_lv_trigo_sin(start_angle) * rin) >> 15) - extra_area;
            inv_area.x2 = x + ((_lv_trigo_sin(start_angle + 90) * rout) >> 15) + extra_area;

            inv_area.y2 = y + ((_lv_trigo_sin(end_angle) * rout) >> 15) + extra_area;
            inv_area.x1 = x + ((_lv_trigo_sin(end_angle + 90) * rin) >> 15) - extra_area;

            lv_obj_invalidate_area(arc, &inv_area);
        }
        else if(start_quarter == 1) {
            inv_area.y2 = y + ((_lv_trigo_sin(start_angle) * rout) >> 15) + extra_area;
            inv_area.x2 = x + ((_lv_trigo_sin(start_angle + 90) * rin) >> 15) + extra_area;

            inv_area.y1 = y + ((_lv_trigo_sin(end_angle) * rin) >> 15) - extra_area;
            inv_area.x1 = x + ((_lv_trigo_sin(end_angle + 90) * rout) >> 15) - extra_area;

            lv_obj_invalidate_area(arc, &inv_area);
        }
        else if(start_quarter == 2) {
            inv_area.x1 = x + ((_lv_trigo_sin(start_angle + 90) * rout) >> 15) - extra_area;
            inv_area.y2 = y + ((_lv_trigo_sin(start_angle) * rin) >> 15) + extra_area;

            inv_area.y1 = y + ((_lv_trigo_sin(end_angle) * rout) >> 15) - extra_area;
            inv_area.x2 = x + ((_lv_trigo_sin(end_angle + 90) * rin) >> 15) + extra_area;

            lv_obj_invalidate_area(arc, &inv_area);
        }
        else if(start_quarter == 3) {
             
            inv_area.x1 = x + ((_lv_trigo_sin(start_angle + 90) * rin) >> 15) - extra_area;
            inv_area.y1 = y + ((_lv_trigo_sin(start_angle) * rout) >> 15) - extra_area;

            inv_area.x2 = x + ((_lv_trigo_sin(end_angle + 90) * rout) >> 15) + extra_area;
            inv_area.y2 = y + ((_lv_trigo_sin(end_angle) * rin) >> 15) + extra_area;
        ...
        inv_area.x1 = x + ((_lv_trigo_sin(end_angle + 90) * rout) >> 15) - extra_area;
        inv_area.y1 = y + ((((_lv_trigo_sin(end_angle)) < (_lv_trigo_sin(start_angle)) ? (_lv_trigo_sin(end_angle)) : (_lv_trigo_sin(start_angle)))  * rin) >> 15) - extra_area;
        ...
        inv_area.y1 = y + ((_lv_trigo_sin(end_angle) * rout) >> 15) - extra_area;
        inv_area.x2 = x + ((((_lv_trigo_sin(start_angle + 90)) > (_lv_trigo_sin(end_angle + 90)) ? (_lv_trigo_sin(start_angle + 90)) : (_lv_trigo_sin(end_angle + 90))) * rin) >> 15) + extra_area;
        ...
        inv_area.x2 = x + ((_lv_trigo_sin(end_angle + 90) * rout) >> 15) + extra_area;
        inv_area.y2 = y + (((_lv_trigo_sin(end_angle) * rin) > (_lv_trigo_sin(start_angle) * rin) ? (_lv_trigo_sin(end_angle) * rin) : (_lv_trigo_sin(start_angle) * rin)) >> 15) + extra_area;

        lv_obj_invalidate_area(arc, &inv_area);
    }
    else if(start_quarter == 3 && end_quarter == 0) {
        inv_area.x1 = x + ((((_lv_trigo_sin(end_angle + 90)) < (_lv_trigo_sin(start_angle + 90)) ? (_lv_trigo_sin(end_angle + 90)) : (_lv_trigo_sin(start_angle + 90))) * rin) >> 15) - extra_area;
...
static void get_center(lv_obj_t * arc, lv_point_t * center, lv_coord_t * arc_r)
{
    ...
    lv_coord_t r = (((lv_obj_get_width(arc) - left_bg - right_bg) < (lv_obj_get_height(arc) - top_bg - bottom_bg) ? (lv_obj_get_width(arc) - left_bg - right_bg) : (lv_obj_get_height(arc) - top_bg - bottom_bg))) / 2;
...
static void get_knob_area(lv_obj_t * arc, const lv_point_t * center, lv_coord_t r, lv_area_t * knob_area)
{
    ...
    lv_coord_t knob_x = (r * _lv_trigo_sin(angle + 90)) >> 15;
    lv_coord_t knob_y = (r * _lv_trigo_sin(angle)) >> 15;
