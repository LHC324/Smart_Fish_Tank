#line 1 "..\\GUI\\lvgl_src\\lv_widgets\\lv_bar.c"
...
#line 12 "..\\GUI\\lvgl_src\\lv_widgets\\lv_bar.c"


#line 15 "..\\GUI\\lvgl_src\\lv_widgets\\lv_bar.c"
#line 16 "..\\GUI\\lvgl_src\\lv_widgets\\lv_bar.c"
...
#line 17 "..\\GUI\\lvgl_src\\lv_widgets\\lv_bar.c"
#line 18 "..\\GUI\\lvgl_src\\lv_widgets\\lv_bar.c"
#line 19 "..\\GUI\\lvgl_src\\lv_widgets\\lv_bar.c"
...
#line 20 "..\\GUI\\lvgl_src\\lv_widgets\\lv_bar.c"
...
#line 34 "..\\GUI\\lvgl_src\\lv_widgets\\lv_bar.c"
...
lv_obj_t * lv_bar_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    do { if(!((lv_debug_check_null(bar)))) { ; lv_debug_log_error("Out of memory", (uint64_t)((uintptr_t)bar)); while(1); } } while(0);;
    if(bar == 0) return 0;

    if(ancestor_signal == 0) ancestor_signal = lv_obj_get_signal_cb(bar);
    if(ancestor_design_f == 0) ancestor_design_f = lv_obj_get_design_cb(bar);
    ...
    if(copy == 0) {
        ...
        lv_obj_set_click(bar, 0);
        lv_obj_set_size(bar, 60 * 2, 60 / 10);
        lv_bar_set_value(bar, ext->cur_value, 0);
        ...
        lv_obj_refresh_style(bar, LV_OBJ_PART_ALL, 0xFF);
...
void lv_bar_set_value(lv_obj_t * bar, int16_t value, lv_anim_enable_t anim)
{
    do { if(!((lv_debug_check_null(bar)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)bar)); while(1); } } while(0);;
...
int16_t lv_bar_get_value(const lv_obj_t * bar)
{
    ...
    lv_bar_ext_t * ext = lv_obj_get_ext_attr(bar);
    return ((((ext->cur_value_anim). anim_state) != -1) ? ((ext->cur_value_anim). anim_end) : (ext->cur_value));
}
...
int16_t lv_bar_get_start_value(const lv_obj_t * bar)
{
    ...
    return ((((ext->start_value_anim). anim_state) != -1) ? ((ext->start_value_anim). anim_end) : (ext->start_value));
}
...
static void draw_indic(lv_obj_t * bar, const lv_area_t * clip_area)
{
    ...
    if(ext->type == LV_BAR_TYPE_SYMMETRICAL && ext->min_value < 0 && ext->max_value > 0 &&
       ext->start_value == ext->min_value) sym = 1;
    ...
    if(hor && lv_area_get_height(&ext->indic_area) < 4) {
        ext->indic_area.y1 = bar->coords.y1 + (objh / 2) - (4 / 2);
        ext->indic_area.y2 = ext->indic_area.y1 + 4;
    }
    else if(!hor && lv_area_get_width(&ext->indic_area) < 4) {
        ext->indic_area.x1 = bar->coords.x1 + (objw / 2) - (4 / 2);
        ext->indic_area.x2 = ext->indic_area.x1 + 4;
    }
    ...
    if((((ext->start_value_anim). anim_state) != -1)) {
    ...
    if((((ext->cur_value_anim). anim_state) != -1)) {
    ...
    lv_coord_t short_side = ((objw) < (objh) ? (objw) : (objh));
    ...
    if((hor && lv_area_get_width(&ext->indic_area) > bg_radius * 2) ||
       (!hor && lv_area_get_height(&ext->indic_area) > bg_radius * 2)) {
        ...
        draw_indic_dsc.pattern_image = 0;
    ...
    lv_draw_mask_radius_init(&mask_bg_param, &bar->coords, bg_radius, 0);
    int16_t mask_bg_id = lv_draw_mask_add(&mask_bg_param, 0);
    ...
    if(hor && lv_area_get_height(&mask_indic_max_area) < 4) {
        mask_indic_max_area.y1 = bar->coords.y1 + (objh / 2) - (4 / 2);
        mask_indic_max_area.y2 = mask_indic_max_area.y1 + 4;
    }
    else if(!hor && lv_area_get_width(&mask_indic_max_area) < 4) {
        mask_indic_max_area.x1 = bar->coords.x1 + (objw / 2) - (4 / 2);
        mask_indic_max_area.x2 = mask_indic_max_area.x1 + 4;
    }

     
    lv_draw_mask_radius_param_t mask_indic_param;
    lv_draw_mask_radius_init(&mask_indic_param, &ext->indic_area, draw_indic_dsc.radius, 0);
    int16_t mask_indic_id = lv_draw_mask_add(&mask_indic_param, 0);
...
static lv_res_t lv_bar_signal(lv_obj_t * bar, lv_signal_t sign, void * param)
{
    ...
    if(sign == LV_SIGNAL_GET_TYPE) return lv_obj_handle_get_type_signal(param, "lv_bar");
    ...
    if(sign == LV_SIGNAL_REFR_EXT_DRAW_PAD) {
        ...
        bar->ext_draw_pad = ((bar->ext_draw_pad) > (indic_size) ? (bar->ext_draw_pad) : (indic_size));
    ...
    if(sign == LV_SIGNAL_CLEANUP) {
        ...
        lv_bar_ext_t * ext = lv_obj_get_ext_attr(bar);
        lv_anim_del(&ext->cur_value_anim, 0);
        lv_anim_del(&ext->start_value_anim, 0);
