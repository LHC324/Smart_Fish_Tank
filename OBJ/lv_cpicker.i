#line 1 "..\\GUI\\lvgl_src\\lv_widgets\\lv_cpicker.c"
...
#line 11 "..\\GUI\\lvgl_src\\lv_widgets\\lv_cpicker.c"


#line 14 "..\\GUI\\lvgl_src\\lv_widgets\\lv_cpicker.c"
...
#line 15 "..\\GUI\\lvgl_src\\lv_widgets\\lv_cpicker.c"
...
#line 16 "..\\GUI\\lvgl_src\\lv_widgets\\lv_cpicker.c"
...
#line 17 "..\\GUI\\lvgl_src\\lv_widgets\\lv_cpicker.c"
...
#line 18 "..\\GUI\\lvgl_src\\lv_widgets\\lv_cpicker.c"
#line 19 "..\\GUI\\lvgl_src\\lv_widgets\\lv_cpicker.c"
...
static _Bool lv_cpicker_hit(lv_obj_t * cpicker, const lv_point_t * p);
...
lv_obj_t * lv_cpicker_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    do { if(!((lv_debug_check_null(cpicker)))) { ; lv_debug_log_error("Out of memory", (uint64_t)((uintptr_t)cpicker)); while(1); } } while(0);;
    if(cpicker == 0) return 0;

    if(ancestor_signal == 0) ancestor_signal = lv_obj_get_signal_cb(cpicker);
    if(ancestor_design == 0) ancestor_design = lv_obj_get_design_cb(cpicker);
    ...
    ext->type = LV_CPICKER_TYPE_DISC;
    ext->hsv = ((lv_color_hsv_t){0, 100, 100});
    ext->knob.colored = 1;
    ...
    if(copy == 0) {
        lv_obj_set_size(cpicker, 60 * 2, 60 * 2);
        lv_obj_add_protect(cpicker, LV_PROTECT_PRESS_LOST);
        lv_obj_set_adv_hittest(cpicker, 1);
        ...
        lv_obj_refresh_style(cpicker, LV_OBJ_PART_ALL, 0xFF);
    }
...
void lv_cpicker_set_type(lv_obj_t * cpicker, lv_cpicker_type_t type)
{
    do { if(!((lv_debug_check_null(cpicker)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)cpicker)); while(1); } } while(0);;
...
_Bool lv_cpicker_set_hsv(lv_obj_t * cpicker, lv_color_hsv_t hsv)
{
    ...
    if(ext->hsv.h == hsv.h && ext->hsv.s == hsv.s && ext->hsv.v == hsv.v) return 0;
...
static void draw_disc_grad(lv_obj_t * cpicker, const lv_area_t * mask)
{
    ...
    line_dsc.width = (r * 628 / (360 / 3)) / 100;
    line_dsc.width += 2;
    uint16_t i;
    ...
    lv_draw_mask_radius_init(&mask_out_param, &mask_area_out, (0x7FFF), 0);
    ...
    for(i = 0; i <= 360; i += 3) {
        ...
        lv_point_t p[2];
        p[0].x = cx + (r * _lv_trigo_sin(i) >> 15);
        p[0].y = cy + (r * _lv_trigo_sin(i + 90) >> 15);
        p[1].x = cx + ((r - cir_w_extra) * _lv_trigo_sin(i) >> 15);
        p[1].y = cy + ((r - cir_w_extra) * _lv_trigo_sin(i + 90) >> 15);
...
static void draw_rect_grad(lv_obj_t * cpicker, const lv_area_t * mask)
{
    ...
    uint16_t i_step = ((3) > (360 / grad_w) ? (3) : (360 / grad_w));
    bg_dsc.radius = 0;
    ...
    for(i = 0; i < 360; i += i_step) {
        ...
        rect_area.x1 = ((grad_area . x1 + xi) < (grad_area . x1 + grad_w - i_step) ? (grad_area . x1 + xi) : (grad_area . x1 + grad_w - i_step));
        rect_area.y1 = grad_area.y1;
        rect_area.x2 = ((grad_area . x1 + xi2) < (grad_area . x1 + grad_w - i_step) ? (grad_area . x1 + xi2) : (grad_area . x1 + grad_w - i_step));
...
static lv_res_t lv_cpicker_signal(lv_obj_t * cpicker, lv_signal_t sign, void * param)
{
    ...
    if(sign == LV_SIGNAL_GET_TYPE) return lv_obj_handle_get_type_signal(param, "lv_cpicker");
    ...
    if(sign == LV_SIGNAL_CLEANUP) {
        ...
        lv_coord_t knob_pad = (((((left) > (right) ? (left) : (right))) > (((top) > (bottom) ? (top) : (bottom))) ? (((left) > (right) ? (left) : (right))) : (((top) > (bottom) ? (top) : (bottom))))) + 2;

        if(ext->type == LV_CPICKER_TYPE_DISC) {
            cpicker->ext_draw_pad = ((cpicker->ext_draw_pad) > (knob_pad) ? (cpicker->ext_draw_pad) : (knob_pad));
        }
        else {
        ...
        if(c == LV_KEY_RIGHT || c == LV_KEY_UP) {
            ...
            if(lv_cpicker_set_hsv(cpicker, hsv_cur)) {
                res = lv_event_send(cpicker, LV_EVENT_VALUE_CHANGED, 0);
                if(res != LV_RES_OK) return res;
            }
        }
        ...
        if((((p . x - ext->last_press_point . x) > 0 ? (p . x - ext->last_press_point . x) : (-(p . x - ext->last_press_point . x))) > indev->driver.drag_limit / 2) ||
           (((p . y - ext->last_press_point . y) > 0 ? (p . y - ext->last_press_point . y) : (-(p . y - ext->last_press_point . y))) > indev->driver.drag_limit / 2)) {
            ext->last_change_time = lv_tick_get();
...
static _Bool lv_cpicker_hit(lv_obj_t * cpicker, const lv_point_t * p)
{
    _Bool is_point_on_coords = lv_obj_is_point_on_coords(cpicker, p);
    ...
    if(_lv_area_is_point_on(&cpicker->coords, p, (0x7FFF))) return 1;
    else return 0;
}
...
static void refr_knob_pos(lv_obj_t * cpicker)
{
    ...
    if(ext->type == LV_CPICKER_TYPE_RECT) {
        ...
        ext->knob.pos.x = (((int32_t)r * _lv_trigo_sin(angle)) >> 15);
        ext->knob.pos.y = (((int32_t)r * _lv_trigo_sin(angle + 90)) >> 15);
...
static lv_res_t double_click_reset(lv_obj_t * cpicker)
{
    ...
    if(lv_tick_elaps(ext->last_click_time) < indev->driver.long_press_time) {
        ...
        if(lv_cpicker_set_hsv(cpicker, hsv_cur)) {
            lv_res_t res = lv_event_send(cpicker, LV_EVENT_VALUE_CHANGED, 0);
            if(res != LV_RES_OK) return res;
        }
    }
