#line 1 "..\\GUI\\lvgl_src\\lv_widgets\\lv_textarea.c"
...
#line 10 "..\\GUI\\lvgl_src\\lv_widgets\\lv_textarea.c"


#line 13 "..\\GUI\\lvgl_src\\lv_widgets\\lv_textarea.c"
#line 14 "..\\GUI\\lvgl_src\\lv_widgets\\lv_textarea.c"
#line 15 "..\\GUI\\lvgl_src\\lv_widgets\\lv_textarea.c"
...
#line 16 "..\\GUI\\lvgl_src\\lv_widgets\\lv_textarea.c"
#line 17 "..\\GUI\\lvgl_src\\lv_widgets\\lv_textarea.c"
...
#line 18 "..\\GUI\\lvgl_src\\lv_widgets\\lv_textarea.c"
#line 19 "..\\GUI\\lvgl_src\\lv_widgets\\lv_textarea.c"
#line 20 "..\\GUI\\lvgl_src\\lv_widgets\\lv_textarea.c"
#line 21 "..\\GUI\\lvgl_src\\lv_widgets\\lv_textarea.c"
...
static _Bool char_is_accepted(lv_obj_t * ta, uint32_t c);
...
lv_obj_t * lv_textarea_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    do { if(!((lv_debug_check_null(ta)))) { ; lv_debug_log_error("Out of memory", (uint64_t)((uintptr_t)ta)); while(1); } } while(0);;
    if(ta == 0) return 0;

    if(ancestor_signal == 0) ancestor_signal = lv_obj_get_signal_cb(ta);
    if(ancestor_design == 0) ancestor_design = lv_obj_get_design_cb(ta);
    if(scrl_signal == 0) scrl_signal = lv_obj_get_signal_cb(lv_page_get_scrollable(ta));
    if(scrl_design == 0) scrl_design = lv_obj_get_design_cb(lv_page_get_scrollable(ta));
    ...
    ext->pwd_mode          = 0;
    ext->pwd_tmp           = 0;
    ext->pwd_show_time     = 1500;
    ext->accapted_chars    = 0;
    ext->max_length        = 0;
    ext->cursor.state      = 1;
    ext->cursor.hidden     = 0;
    ext->cursor.blink_time = 400;
    ext->cursor.pos        = 0;
    ...
    if(copy == 0) {
        ...
        ext->label = lv_label_create(ta, 0);

        lv_obj_set_design_cb(ext->page.scrl, lv_textarea_scrollable_design);

        lv_label_set_long_mode(ext->label, LV_LABEL_LONG_BREAK);
        lv_label_set_text(ext->label, "Text area");
        lv_obj_set_click(ext->label, 0);
        lv_obj_set_size(ta, (2 * 60), (1 * 60));
        ...
        if(copy_ext->pwd_tmp) {
            ...
            do { if(!((lv_debug_check_null(ext->pwd_tmp)))) { ; lv_debug_log_error("Out of memory", (uint64_t)((uintptr_t)ext->pwd_tmp)); while(1); } } while(0);;
            if(ext->pwd_tmp == 0) return 0;

            _lv_memcpy(ext->pwd_tmp, copy_ext->pwd_tmp, len);
        }

        if(copy_ext->one_line) lv_textarea_set_one_line(ta, 1);

         
        lv_obj_refresh_style(ta, LV_OBJ_PART_ALL, 0xFF);
    }
    ...
    if(ext->cursor.blink_time) {
        ...
        lv_anim_set_repeat_count(&a, 0xFFFF);
        lv_anim_start(&a);
    }
...
void lv_textarea_add_char(lv_obj_t * ta, uint32_t c)
{
    ...
    uint32_t c_uni = _lv_txt_encoded_next((const char *)&c, 0);

    if(char_is_accepted(ta, c_uni) == 0) {
        ;
    ...
    _Bool edge_flash_en = lv_textarea_get_edge_flash(ta);
    lv_textarea_set_edge_flash(ta, 0);
    ...
    lv_label_ins_text(ext->label, ext->cursor.pos, letter_buf);  
    ...
    lv_event_send(ta, LV_EVENT_VALUE_CHANGED, 0);
}
...
{
    do { if(!((lv_debug_check_null(ta)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)ta)); while(1); } } while(0);;
    do { if(!((lv_debug_check_null(txt)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)txt)); while(1); } } while(0);;
...
void lv_textarea_set_text(lv_obj_t * ta, const char * txt)
{
    ...
    if(lv_textarea_get_accepted_chars(ta) || lv_textarea_get_max_length(ta)) {
        ...
        lv_textarea_set_cursor_pos(ta, (0x7FFF));
        if(ext->pwd_mode != 0) {
            ext->pwd_tmp[0] = '\0';  
        }
        uint32_t i = 0;
...
void lv_textarea_set_placeholder_text(lv_obj_t * ta, const char * txt)
{
    ...
    if(txt_len == 0) {
        ...
        do { if(!((lv_debug_check_null(ext->placeholder_txt)))) { ; lv_debug_log_error("Out of memory", (uint64_t)((uintptr_t)ext->placeholder_txt)); while(1); } } while(0);;
        if(ext->placeholder_txt == 0) {
            ;
            return;
        }
...
void lv_textarea_set_cursor_pos(lv_obj_t * ta, int32_t pos)
{
    ...
    if(pos > (int32_t)len || pos == (0x7FFF)) pos = len;
...
#line 881 "..\\GUI\\lvgl_src\\lv_widgets\\lv_textarea.c"
    (void)ta;  
    (void)en;  
...
_Bool lv_textarea_get_cursor_click_pos(lv_obj_t * ta)
{
    ...
    lv_textarea_ext_t * ext = lv_obj_get_ext_attr(ta);
    return ext->cursor.click_pos ? 1 : 0;
}
...
#line 1110 "..\\GUI\\lvgl_src\\lv_widgets\\lv_textarea.c"
    (void)ta;  
    return 0;
...
#line 1181 "..\\GUI\\lvgl_src\\lv_widgets\\lv_textarea.c"
    (void)ta;  
                                                     ...
                                                     lv_design_mode_t mode)
{
    if(mode == LV_DESIGN_COVER_CHK) {
        ...
        if(txt[0] == '\0' && ext->placeholder_txt && ext->placeholder_txt[0] != 0) {
            ...
            lv_draw_label(&scrl->coords, clip_area, &ph_dsc, ext->placeholder_txt, 0);
        }
        ...
        if(cur_dsc.bg_opa == LV_OPA_COVER) {
            ...
            lv_draw_label(&cur_area, clip_area, &cur_label_dsc, letter_buf, 0);
        }
    }
...
static lv_res_t lv_textarea_signal(lv_obj_t * ta, lv_signal_t sign, void * param)
{
    ...
    if(sign == LV_SIGNAL_GET_TYPE) return lv_obj_handle_get_type_signal(param, "lv_textarea");

    lv_textarea_ext_t * ext = lv_obj_get_ext_attr(ta);
    if(sign == LV_SIGNAL_CLEANUP) {
        if(ext->pwd_tmp != 0) lv_mem_free(ext->pwd_tmp);
        if(ext->placeholder_txt != 0) lv_mem_free(ext->placeholder_txt);
        ...
        if(c == LV_KEY_RIGHT)
            ...
            lv_event_send(ta, LV_EVENT_APPLY, 0);
        else {
...
static lv_res_t lv_textarea_scrollable_signal(lv_obj_t * scrl, lv_signal_t sign, void * param)
{
    ...
    if(sign == LV_SIGNAL_REFR_EXT_DRAW_PAD) {
        ...
        const lv_font_t * font = lv_obj_get_style_text_font(ta, LV_TEXTAREA_PART_BG);
        lv_coord_t font_h              = lv_font_get_line_height(font);
        scrl->ext_draw_pad             = ((scrl->ext_draw_pad) > (line_space + font_h) ? (scrl->ext_draw_pad) : (line_space + font_h));
    }
    else if(sign == LV_SIGNAL_COORD_CHG) {
...
static void pwd_char_hider(lv_obj_t * ta)
{
    ...
    if(ext->pwd_mode != 0) {
        ...
        _Bool has_bullet;
        has_bullet = lv_font_get_glyph_dsc(font, &g, 0x2022, 0);
        const char * bullet;
        if(has_bullet) bullet = "\xE2\x80\xA2";
        else bullet = "*";
...
static _Bool char_is_accepted(lv_obj_t * ta, uint32_t c)
{
    ...
    if(ext->accapted_chars == 0 && ext->max_length == 0) return 1;
...
static void refr_cursor_area(lv_obj_t * ta)
{
    ...
    uint32_t letter = _lv_txt_encoded_next(&txt[byte_pos], 0);
    ...
    if(letter_pos.x + ext->label->coords.x1 + letter_w > ext->label->coords.x2 && ext->one_line == 0 &&
       lv_label_get_align(ext->label) != LV_LABEL_ALIGN_RIGHT) {
        ...
        if(letter != '\0') {
            ...
            letter = _lv_txt_encoded_next(&txt[byte_pos], 0);
        }
...
#line 1895 "..\\GUI\\lvgl_src\\lv_widgets\\lv_textarea.c"
...
static lv_res_t insert_handler(lv_obj_t * ta, const char * txt)
{
    ...
    if(ta_insert_replace) {
        if(ta_insert_replace[0] == '\0') return LV_RES_INV;  
