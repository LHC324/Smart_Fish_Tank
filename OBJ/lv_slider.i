#line 1 "..\\GUI\\lvgl_src\\lv_widgets\\lv_slider.c"
...
#line 11 "..\\GUI\\lvgl_src\\lv_widgets\\lv_slider.c"


#line 14 "..\\GUI\\lvgl_src\\lv_widgets\\lv_slider.c"
#line 15 "..\\GUI\\lvgl_src\\lv_widgets\\lv_slider.c"
#line 16 "..\\GUI\\lvgl_src\\lv_widgets\\lv_slider.c"
#line 17 "..\\GUI\\lvgl_src\\lv_widgets\\lv_slider.c"
...
#line 18 "..\\GUI\\lvgl_src\\lv_widgets\\lv_slider.c"
#line 19 "..\\GUI\\lvgl_src\\lv_widgets\\lv_slider.c"
...
#line 20 "..\\GUI\\lvgl_src\\lv_widgets\\lv_slider.c"
...
static void lv_slider_position_knob(lv_obj_t * slider, lv_area_t * knob_area, lv_coord_t knob_size, _Bool hor);
...
lv_obj_t * lv_slider_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    do { if(!((lv_debug_check_null(slider)))) { ; lv_debug_log_error("Out of memory", (uint64_t)((uintptr_t)slider)); while(1); } } while(0);;
    if(slider == 0) return 0;

    if(ancestor_design_f == 0) ancestor_design_f = lv_obj_get_design_cb(slider);
    if(ancestor_signal == 0) ancestor_signal = lv_obj_get_signal_cb(slider);
    ...
    if(copy == 0) {
        lv_obj_set_click(slider, 1);
        lv_obj_add_protect(slider, LV_PROTECT_PRESS_LOST);
        lv_obj_set_ext_click_area(slider, 0, 0, 60 / 10, 60 / 10);

        lv_theme_apply(slider, LV_THEME_SLIDER);
        lv_obj_set_height(slider, 60 / 15);
    }
        ...
        lv_obj_refresh_style(slider, LV_OBJ_PART_ALL, 0xFF);
    }
...
int16_t lv_slider_get_value(const lv_obj_t * slider)
{
    do { if(!((lv_debug_check_null(slider)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)slider)); while(1); } } while(0);;
...
static lv_design_res_t lv_slider_design(lv_obj_t * slider, const lv_area_t * clip_area, lv_design_mode_t mode)
{
    ...
    if(mode == LV_DESIGN_COVER_CHK) {
        ...
        _Bool hor = objw >= objh ? 1 : 0;
        lv_coord_t knob_size = hor ? objh : objw;
        _Bool sym = 0;
        if(ext->bar.type == LV_BAR_TYPE_SYMMETRICAL && ext->bar.min_value < 0 && ext->bar.max_value > 0) sym = 1;
        ...
        if(hor) {
            if(!sym) {
                knob_area.x1 = (hor ? (base_dir == LV_BIDI_DIR_RTL ? ext->bar . indic_area . x1 : ext->bar . indic_area . x2) : (base_dir == LV_BIDI_DIR_RTL ? ext->bar . indic_area . y1 : ext->bar . indic_area . y2));
            }
            else {
...
static lv_res_t lv_slider_signal(lv_obj_t * slider, lv_signal_t sign, void * param)
{
    ...
    if(sign == LV_SIGNAL_GET_TYPE) return lv_obj_handle_get_type_signal(param, "lv_slider");
    ...
    if(sign == LV_SIGNAL_HIT_TEST) {
        ...
        if((info->result == 0) && (type == LV_SLIDER_TYPE_RANGE)) {
    ...
    if(sign == LV_SIGNAL_PRESSED) {
        ...
        if(type == LV_SLIDER_TYPE_NORMAL || type == LV_SLIDER_TYPE_SYMMETRICAL) {
            ...
            _Bool hor = lv_obj_get_width(slider) >= lv_obj_get_height(slider);
            lv_bidi_dir_t base_dir = lv_obj_get_base_dir(slider);
            ...
            if(hor) {
                if((base_dir != LV_BIDI_DIR_RTL && p.x > ext->right_knob_area.x2) || (base_dir == LV_BIDI_DIR_RTL &&
                    ...
                    dist_left = (((ext->left_knob_area . x1 + (ext->left_knob_area . x2 - ext->left_knob_area . x1) / 2) - p . x) > 0 ? ((ext->left_knob_area . x1 + (ext->left_knob_area . x2 - ext->left_knob_area . x1) / 2) - p . x) : (-((ext->left_knob_area . x1 + (ext->left_knob_area . x2 - ext->left_knob_area . x1) / 2) - p . x)));
                    dist_right = (((ext->right_knob_area . x1 + (ext->right_knob_area . x2 - ext->right_knob_area . x1) / 2) - p . x) > 0 ? ((ext->right_knob_area . x1 + (ext->right_knob_area . x2 - ext->right_knob_area . x1) / 2) - p . x) : (-((ext->right_knob_area . x1 + (ext->right_knob_area . x2 - ext->right_knob_area . x1) / 2) - p . x)));
                ...
                if(p.y < ext->right_knob_area.y1) {
                    ...
                    dist_left = (((ext->left_knob_area . y1 + (ext->left_knob_area . y2 - ext->left_knob_area . y1) / 2) - p . y) > 0 ? ((ext->left_knob_area . y1 + (ext->left_knob_area . y2 - ext->left_knob_area . y1) / 2) - p . y) : (-((ext->left_knob_area . y1 + (ext->left_knob_area . y2 - ext->left_knob_area . y1) / 2) - p . y)));
                    dist_right = (((ext->right_knob_area . y1 + (ext->right_knob_area . y2 - ext->right_knob_area . y1) / 2) - p . y) > 0 ? ((ext->right_knob_area . y1 + (ext->right_knob_area . y2 - ext->right_knob_area . y1) / 2) - p . y) : (-((ext->right_knob_area . y1 + (ext->right_knob_area . y2 - ext->right_knob_area . y1) / 2) - p . y)));
    ...
    else if(sign == LV_SIGNAL_PRESSING && ext->value_to_set != 0) {
        lv_indev_get_point(param, &p);
        lv_bidi_dir_t base_dir = lv_obj_get_base_dir(slider);
        ...
        if(w >= h) {
            ...
            if(base_dir == LV_BIDI_DIR_RTL) {
                new_value = (slider->coords.x2 - bg_right) - p.x;  
            }
            else {
                new_value = p.x - (slider->coords.x1 + bg_left);  
            }
            ...
            new_value = p.y - (slider->coords.y2 + bg_bottom);  
        ...
        if(*ext->value_to_set != new_value) {
            *ext->value_to_set = new_value;
            ...
            res = lv_event_send(slider, LV_EVENT_VALUE_CHANGED, 0);
            if(res != LV_RES_OK) return res;
        }
        ...
        if(lv_obj_get_width(slider) != lv_area_get_width(param) ||
           lv_obj_get_height(slider) != lv_area_get_height(param)) {
            slider->signal_cb(slider, LV_SIGNAL_REFR_EXT_DRAW_PAD, 0);
        }
    }
        ...
        lv_coord_t knob_size = ((lv_obj_get_width(slider)) < (lv_obj_get_height(slider)) ? (lv_obj_get_width(slider)) : (lv_obj_get_height(slider))) >> 1;
        knob_size += ((((knob_left) > (knob_right) ? (knob_left) : (knob_right))) > (((knob_bottom) > (knob_top) ? (knob_bottom) : (knob_top))) ? (((knob_left) > (knob_right) ? (knob_left) : (knob_right))) : (((knob_bottom) > (knob_top) ? (knob_bottom) : (knob_top))));
        knob_size += 2;          

        knob_size += lv_obj_get_draw_rect_ext_pad_size(slider, LV_SLIDER_PART_KNOB);

         
        slider->ext_draw_pad = ((slider->ext_draw_pad) > (knob_size) ? (slider->ext_draw_pad) : (knob_size));
