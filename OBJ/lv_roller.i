#line 1 "..\\GUI\\lvgl_src\\lv_widgets\\lv_roller.c"
...
#line 10 "..\\GUI\\lvgl_src\\lv_widgets\\lv_roller.c"


#line 13 "..\\GUI\\lvgl_src\\lv_widgets\\lv_roller.c"
#line 14 "..\\GUI\\lvgl_src\\lv_widgets\\lv_roller.c"
#line 15 "..\\GUI\\lvgl_src\\lv_widgets\\lv_roller.c"
...
#line 16 "..\\GUI\\lvgl_src\\lv_widgets\\lv_roller.c"
...
lv_obj_t * lv_roller_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    do { if(!((lv_debug_check_null(roller)))) { ; lv_debug_log_error("Out of memory", (uint64_t)((uintptr_t)roller)); while(1); } } while(0);;
    if(roller == 0) return 0;

    if(ancestor_scrl_signal == 0) ancestor_scrl_signal = lv_obj_get_signal_cb(lv_page_get_scrollable(roller));
    if(ancestor_signal == 0) ancestor_signal = lv_obj_get_signal_cb(roller);
    ...
    if(copy == 0) {
        ...
        lv_obj_t * label = lv_label_create(roller, 0);
        lv_label_set_align(label, LV_LABEL_ALIGN_CENTER);
        if(ancestor_label_design == 0) ancestor_label_design = lv_obj_get_design_cb(label);
        lv_obj_set_design_cb(label, lv_roller_label_design);
        lv_obj_t * scrl = lv_page_get_scrollable(roller);
        lv_obj_set_drag(scrl, 1);
        lv_page_set_scrollable_fit2(roller, LV_FIT_PARENT, LV_FIT_NONE);  
        lv_roller_set_anim_time(roller, 200);
        lv_roller_set_options(roller, "Option 1\nOption 2\nOption 3\nOption 4\nOption 5", LV_ROLLER_MODE_NORMAL);

        lv_obj_set_signal_cb(scrl, lv_roller_scrl_signal);

        lv_obj_clean_style_list(roller, LV_PAGE_PART_SCROLLABLE);  
        ...
        lv_obj_refresh_style(roller, LV_OBJ_PART_ALL, 0xFF);
    }
...
void lv_roller_set_options(lv_obj_t * roller, const char * options, lv_roller_mode_t mode)
{
    do { if(!((lv_debug_check_null(roller)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)roller)); while(1); } } while(0);;
    ...
    if(mode == LV_ROLLER_MODE_NORMAL) {
        ...
        char * opt_extra = _lv_mem_buf_get(opt_len * 7);
        uint8_t i;
        for(i = 0; i < 7; i++) {
            strcpy(&opt_extra[opt_len * i], options);
            opt_extra[opt_len * (i + 1) - 1] = '\n';
        }
        opt_extra[opt_len * 7 - 1] = '\0';
        lv_label_set_text(label, opt_extra);
        _lv_mem_buf_release(opt_extra);

        ext->sel_opt_id     = ((7 / 2) + 0) * ext->option_cnt;

        ext->option_cnt = ext->option_cnt * 7;
    }
...
void lv_roller_set_selected(lv_obj_t * roller, uint16_t sel_opt, lv_anim_enable_t anim)
{
    ...
    if(ext->mode == LV_ROLLER_MODE_INIFINITE) {
        ...
        uint16_t page = ext->sel_opt_id / 7;

        

 
        if(page != 0) {
            sel_opt_signed -= page * 7;
        }

        sel_opt = page * 7 + sel_opt_signed;
    }
...
uint16_t lv_roller_get_selected(const lv_obj_t * roller)
{
    ...
    lv_roller_ext_t * ext = lv_obj_get_ext_attr(roller);
    if(ext->mode == LV_ROLLER_MODE_INIFINITE) {
        uint16_t real_id_cnt = ext->option_cnt / 7;
...
static lv_design_res_t lv_roller_design(lv_obj_t * roller, const lv_area_t * clip_area, lv_design_mode_t mode)
{
    if(mode == LV_DESIGN_COVER_CHK) {
        ...
        if((font_h & 0x1) && (line_space & 0x1)) rect_area.y1--;  
        ...
        if((bg_font_h & 0x1) && (label_dsc.line_space & 0x1)) rect_area.y1--;  
        ...
        if(area_ok) {
            ...
            lv_draw_label(&label_sel_area, &mask_sel, &label_dsc, lv_label_get_text(label), 0);
        }
    }
...
static lv_res_t lv_roller_signal(lv_obj_t * roller, lv_signal_t sign, void * param)
{
    ...
    if(sign == LV_SIGNAL_GET_TYPE) return lv_obj_handle_get_type_signal(param, "lv_roller");
    ...
    if(sign == LV_SIGNAL_STYLE_CHG) {
        ...
        if(label) lv_signal_send(label, LV_SIGNAL_STYLE_CHG, 0);
        refr_height(roller);
        refr_width(roller);
        refr_position(roller, 0);
    }
        ...
        if(c == LV_KEY_RIGHT || c == LV_KEY_DOWN) {
            if(ext->sel_opt_id + 1 < ext->option_cnt) {
                uint16_t ori_id = ext->sel_opt_id_ori;  
                lv_roller_set_selected(roller, ext->sel_opt_id + 1, 1);
...
static void draw_bg(lv_obj_t * roller, const lv_area_t * clip_area)
{
    ...
    half_mask.x1 -= roller->ext_draw_pad;  
...
static void refr_width(lv_obj_t * roller)
{
    ...
    switch(lv_label_get_align(label)) {
        case LV_LABEL_ALIGN_LEFT:
            lv_obj_align(label, 0, LV_ALIGN_IN_LEFT_MID, 0, 0);
            break;
        case LV_LABEL_ALIGN_CENTER:
            lv_obj_align(label, 0, LV_ALIGN_CENTER, 0, 0);
            break;
        case LV_LABEL_ALIGN_RIGHT:
            lv_obj_align(label, 0, LV_ALIGN_IN_RIGHT_MID, 0, 0);
            break;
    }


    if(lv_roller_get_auto_fit(roller) == 0) return;
    ...
    if(base_font != sel_font) {
        ...
        lv_point_t p;
        _lv_txt_get_size(&p, lv_label_get_text(label), sel_font, letter_sp, line_sp, ((lv_coord_t)((uint32_t)((uint32_t)1 << (8 * sizeof(lv_coord_t) - 1)) - 1000)), LV_TXT_FLAG_NONE);
...
static void inf_normalize(void * scrl)
{
    ...
    if(ext->mode == LV_ROLLER_MODE_INIFINITE) {
        ...
        ext->sel_opt_id += (7 / 2) * real_id_cnt;  

        ext->sel_opt_id_ori = ext->sel_opt_id % real_id_cnt;
        ext->sel_opt_id_ori += (7 / 2) * real_id_cnt;  
...
static lv_obj_t * get_label(const lv_obj_t * roller)
{
    lv_obj_t * scrl = lv_page_get_scrollable(roller);
    if(scrl == 0) return 0;    
    return lv_obj_get_child(scrl, 0);
}
