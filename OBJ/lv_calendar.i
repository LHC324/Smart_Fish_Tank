#line 1 "..\\GUI\\lvgl_src\\lv_widgets\\lv_calendar.c"
...
#line 10 "..\\GUI\\lvgl_src\\lv_widgets\\lv_calendar.c"


#line 13 "..\\GUI\\lvgl_src\\lv_widgets\\lv_calendar.c"
...
#line 14 "..\\GUI\\lvgl_src\\lv_widgets\\lv_calendar.c"
#line 15 "..\\GUI\\lvgl_src\\lv_widgets\\lv_calendar.c"
...
#line 16 "..\\GUI\\lvgl_src\\lv_widgets\\lv_calendar.c"
...
#line 17 "..\\GUI\\lvgl_src\\lv_widgets\\lv_calendar.c"
...
#line 18 "..\\GUI\\lvgl_src\\lv_widgets\\lv_calendar.c"
#line 19 "..\\GUI\\lvgl_src\\lv_widgets\\lv_calendar.c"
...
static _Bool calculate_touched_day(lv_obj_t * calendar, const lv_point_t * touched_point);
...
static _Bool is_highlighted(lv_obj_t * calendar, day_draw_state_t draw_state, int32_t year, int32_t month, int32_t day);
static _Bool is_pressed(lv_obj_t * calendar, day_draw_state_t draw_state, int32_t year, int32_t month, int32_t day);
...
lv_obj_t * lv_calendar_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    do { if(!((lv_debug_check_null(calendar)))) { ; lv_debug_log_error("Out of memory", (uint64_t)((uintptr_t)calendar)); while(1); } } while(0);;
    ...
    if(ancestor_signal == 0) ancestor_signal = lv_obj_get_signal_cb(calendar);
    if(ancestor_design == 0) ancestor_design = lv_obj_get_design_cb(calendar);
    ...
    if(copy == 0) {
        ...
        lv_obj_set_size(calendar, 5 * 60 / 2, 5 * 60 / 2);
...
void lv_calendar_set_today_date(lv_obj_t * calendar, lv_calendar_date_t * today)
{
    do { if(!((lv_debug_check_null(calendar)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)calendar)); while(1); } } while(0);;
    do { if(!((lv_debug_check_null(today)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)today)); while(1); } } while(0);;
...
{
    do { if(!((lv_debug_check_null(calendar)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)calendar)); while(1); } } while(0);;
    do { if(!((lv_debug_check_null(showed)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)showed)); while(1); } } while(0);;
...
{
    do { if(!((lv_debug_check_null(calendar)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)calendar)); while(1); } } while(0);;
    do { if(!((lv_debug_check_null(highlighted)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)highlighted)); while(1); } } while(0);;
...
{
    do { if(!((lv_debug_check_null(calendar)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)calendar)); while(1); } } while(0);;
    do { if(!((lv_debug_check_null(day_names)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)day_names)); while(1); } } while(0);;
...
{
    do { if(!((lv_debug_check_null(calendar)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)calendar)); while(1); } } while(0);;
    do { if(!((lv_debug_check_null(month_names)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)month_names)); while(1); } } while(0);;
...
lv_calendar_date_t * lv_calendar_get_pressed_date(const lv_obj_t * calendar)
{
    ...
    lv_calendar_ext_t * ext = lv_obj_get_ext_attr(calendar);
    return ext->pressed_date.year != 0 ? &ext->pressed_date : 0;
}
...
static lv_res_t lv_calendar_signal(lv_obj_t * calendar, lv_signal_t sign, void * param)
{
    ...
    if(sign == LV_SIGNAL_GET_TYPE) return lv_obj_handle_get_type_signal(param, "lv_calendar");
    ...
    if(sign == LV_SIGNAL_CLEANUP) {
        ...
        if(ext->btn_pressing < 0) {
            ...
            res = lv_event_send(calendar, LV_EVENT_VALUE_CHANGED, 0);
            if(res != LV_RES_OK) return res;
...
static void draw_header(lv_obj_t * calendar, const lv_area_t * mask)
{
    ...
    _lv_obj_disable_style_caching(calendar, 1);
    ...
    lv_draw_label(&header_area, mask, &label_dsc, txt_buf, 0);
    ...
    lv_draw_label_dsc_init(&label_dsc);
    lv_obj_init_draw_label_dsc(calendar, LV_CALENDAR_PART_HEADER, &label_dsc);
    lv_draw_label(&header_area, mask, &label_dsc, "\xef\x81\x93", 0);
    ...
    header_area.x1 = header_area.x2 - header_right - _lv_txt_get_width("\xef\x81\x94", (uint16_t)strlen("\xef\x81\x94"),
                                                                       font, 0, LV_TXT_FLAG_NONE);

    lv_draw_label_dsc_init(&label_dsc);
    lv_obj_init_draw_label_dsc(calendar, LV_CALENDAR_PART_HEADER, &label_dsc);
    lv_draw_label(&header_area, mask, &label_dsc, "\xef\x81\x94", 0);

    calendar->state = state_ori;     
    _lv_obj_disable_style_caching(calendar, 0);
}
...
static void draw_day_names(lv_obj_t * calendar, const lv_area_t * mask)
{
    ...
    lv_coord_t box_size = ((box_w) < (box_h) ? (box_w) : (box_h));
    ...
    for(i = 0; i < 7; i++) {
        ...
        lv_draw_label(&label_area, mask, &label_dsc, get_day_name(calendar, i), 0);
    }
}
...
static void draw_dates(lv_obj_t * calendar, const lv_area_t * clip_area)
{
    ...
    for(week = 0; week < 6; week++) {
        ...
        for(day = 0; day < 7; day++) {
            ...
            lv_draw_label(&label_area, clip_area, &label_dsc, buf, 0);
...
static _Bool is_pressed(lv_obj_t * calendar, day_draw_state_t draw_state, int32_t year, int32_t month, int32_t day)
{
    ...
    if(year == ext->pressed_date.year && month == ext->pressed_date.month && day == ext->pressed_date.day) return 1;
    else return 0;
}
