#line 1 "..\\GUI\\lvgl_src\\lv_widgets\\lv_img.c"
...
#line 10 "..\\GUI\\lvgl_src\\lv_widgets\\lv_img.c"
...
#line 18 "..\\GUI\\lvgl_src\\lv_widgets\\lv_img.c"
...
#line 19 "..\\GUI\\lvgl_src\\lv_widgets\\lv_img.c"
#line 20 "..\\GUI\\lvgl_src\\lv_widgets\\lv_img.c"
#line 21 "..\\GUI\\lvgl_src\\lv_widgets\\lv_img.c"
#line 22 "..\\GUI\\lvgl_src\\lv_widgets\\lv_img.c"
#line 23 "..\\GUI\\lvgl_src\\lv_widgets\\lv_img.c"
#line 24 "..\\GUI\\lvgl_src\\lv_widgets\\lv_img.c"
...
lv_obj_t * lv_img_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    do { if(!((lv_debug_check_null(img)))) { ; lv_debug_log_error("Out of memory", (uint64_t)((uintptr_t)img)); while(1); } } while(0);;
    if(img == 0) return 0;

    if(ancestor_signal == 0) ancestor_signal = lv_obj_get_signal_cb(img);
    ...
    if(copy == 0) {
        ...
        lv_obj_refresh_style(img, LV_OBJ_PART_ALL, 0xFF);
    }
...
#line 163 "..\\GUI\\lvgl_src\\lv_widgets\\lv_img.c"
    ...
    if(src_type == LV_IMG_SRC_VARIABLE) {
        ...
        if(ext->src != src_img) {
            ...
            do { if(!((lv_debug_check_null(new_str)))) { ; lv_debug_log_error("Out of memory", (uint64_t)((uintptr_t)new_str)); while(1); } } while(0);;
            if(new_str == 0) return;
    ...
    if(src_type == LV_IMG_SRC_SYMBOL) {
        ...
        _lv_txt_get_size(&size, src_img, font, letter_space, line_space,
                         ((lv_coord_t)((uint32_t)((uint32_t)1 << (8 * sizeof(lv_coord_t) - 1)) - 1000)), LV_TXT_FLAG_NONE);
        header.w = size.x;
        header.h = size.y;
    }
    ...
    if(lv_img_get_auto_size(img) != 0) {
        lv_obj_set_size(img, ext->w, ext->h);
    }

     
    if(ext->angle || ext->zoom != 256) lv_obj_refresh_ext_draw_pad(img);
...
void lv_img_set_auto_size(lv_obj_t * img, _Bool en)
{
    do { if(!((lv_debug_check_null(img)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)img)); while(1); } } while(0);;

    lv_img_ext_t * ext = lv_obj_get_ext_attr(img);

    ext->auto_size = (en == 0 ? 0 : 1);
}
...
static lv_design_res_t lv_img_design(lv_obj_t * img, const lv_area_t * clip_area, lv_design_mode_t mode)
{
    ...
    if(mode == LV_DESIGN_COVER_CHK) {
        ...
        if(zoom_final == 256) {
            if(_lv_area_is_in(clip_area, &img->coords, 0) == 0) return LV_DESIGN_RES_NOT_COVER;
        }
        else {
            lv_area_t a;
            ...
            if(_lv_area_is_in(clip_area, &a, 0) == 0) return LV_DESIGN_RES_NOT_COVER;
        }
        ...
        if(ext->src_type == LV_IMG_SRC_FILE || ext->src_type == LV_IMG_SRC_VARIABLE) {
            ...
            if(_lv_area_intersect(&clip_real, &clip_real, clip_area) == 0) return LV_DESIGN_RES_OK;
            ...
            lv_draw_label(&img->coords, clip_area, &label_dsc, ext->src, 0);
        }
        else {
             
            ;
            lv_draw_img(&img->coords, clip_area, 0, 0);
        }
    }
...
static lv_res_t lv_img_signal(lv_obj_t * img, lv_signal_t sign, void * param)
{
    ...
    if(sign == LV_SIGNAL_GET_TYPE) return lv_obj_handle_get_type_signal(param, "lv_img");
    ...
    if(sign == LV_SIGNAL_CLEANUP) {
        ...
        if(transf_angle || transf_zoom != 256) {
            lv_area_t a;
            lv_coord_t w = lv_obj_get_width(img);
            lv_coord_t h = lv_obj_get_height(img);
            _lv_img_buf_get_transformed_area(&a, w, h, transf_angle, transf_zoom, &ext->pivot);
            lv_coord_t pad_ori = img->ext_draw_pad;
            img->ext_draw_pad = ((img->ext_draw_pad) > (pad_ori - a . x1) ? (img->ext_draw_pad) : (pad_ori - a . x1));
            img->ext_draw_pad = ((img->ext_draw_pad) > (pad_ori - a . y1) ? (img->ext_draw_pad) : (pad_ori - a . y1));
            img->ext_draw_pad = ((img->ext_draw_pad) > (pad_ori + a . x2 - w) ? (img->ext_draw_pad) : (pad_ori + a . x2 - w));
            img->ext_draw_pad = ((img->ext_draw_pad) > (pad_ori + a . y2 - h) ? (img->ext_draw_pad) : (pad_ori + a . y2 - h));
        }
        ...
        img->ext_draw_pad = ((img->ext_draw_pad) > (left) ? (img->ext_draw_pad) : (left));
        img->ext_draw_pad = ((img->ext_draw_pad) > (right) ? (img->ext_draw_pad) : (right));
        img->ext_draw_pad = ((img->ext_draw_pad) > (top) ? (img->ext_draw_pad) : (top));
        img->ext_draw_pad = ((img->ext_draw_pad) > (bottom) ? (img->ext_draw_pad) : (bottom));
        ...
        if(ext->w == lv_obj_get_width(img) && ext->h == lv_obj_get_height(img) &&
           (zoom != 256 || angle != 0 || ext->pivot.x != ext->w / 2 || ext->pivot.y != ext->h / 2)) {
