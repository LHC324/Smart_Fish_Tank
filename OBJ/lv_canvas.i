#line 1 "..\\GUI\\lvgl_src\\lv_widgets\\lv_canvas.c"
...
#line 10 "..\\GUI\\lvgl_src\\lv_widgets\\lv_canvas.c"
...
#line 11 "..\\GUI\\lvgl_src\\lv_widgets\\lv_canvas.c"
#line 12 "..\\GUI\\lvgl_src\\lv_widgets\\lv_canvas.c"
#line 13 "..\\GUI\\lvgl_src\\lv_widgets\\lv_canvas.c"
#line 14 "..\\GUI\\lvgl_src\\lv_widgets\\lv_canvas.c"
...
#line 15 "..\\GUI\\lvgl_src\\lv_widgets\\lv_canvas.c"
...
#line 16 "..\\GUI\\lvgl_src\\lv_widgets\\lv_canvas.c"
...
lv_obj_t * lv_canvas_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    do { if(!((lv_debug_check_null(new_canvas)))) { ; lv_debug_log_error("Out of memory", (uint64_t)((uintptr_t)new_canvas)); while(1); } } while(0);;
    if(new_canvas == 0) return 0;
    ...
    if(ancestor_signal == 0) ancestor_signal = lv_obj_get_signal_cb(new_canvas);
    if(ancestor_design == 0) ancestor_design = lv_obj_get_design_cb(new_canvas);
...
void lv_canvas_set_buffer(lv_obj_t * canvas, void * buf, lv_coord_t w, lv_coord_t h, lv_img_cf_t cf)
{
    do { if(!((lv_debug_check_null(canvas)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)canvas)); while(1); } } while(0);;
    do { if(!((lv_debug_check_null(buf)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)buf)); while(1); } } while(0);;
...
{
    do { if(!((lv_debug_check_null(canvas)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)canvas)); while(1); } } while(0);;
    do { if(!((lv_debug_check_null(to_copy)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)to_copy)); while(1); } } while(0);;
                         ...
                         int32_t pivot_x, int32_t pivot_y, _Bool antialias)
{
    ...
    do { if(!((lv_debug_check_null(canvas)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)canvas)); while(1); } } while(0);;
    do { if(!((lv_debug_check_null(img)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)img)); while(1); } } while(0);;
    ...
    for(y = -offset_y; y < dest_height - offset_y; y++) {
        for(x = -offset_x; x < dest_width - offset_x; x++) {
            ...
            if(x + offset_x >= 0 && x + offset_x < dest_width && y + offset_y >= 0 && y + offset_y < dest_height) {
                ...
                if(lv_img_cf_has_alpha(img->header.cf) == 0) {
                    ...
                    if(lv_img_cf_has_alpha(ext_dst->dsc.header.cf) == 0) {
                        if(dsc.res.opa < 253) dsc.res.color = lv_color_mix(dsc.res.color, bg_color, dsc.res.opa);
                        lv_img_buf_set_px_color(&ext_dst->dsc, x + offset_x, y + offset_y, dsc.res.color);
                    }
    ...
    lv_obj_invalidate(canvas);
#line 378 "..\\GUI\\lvgl_src\\lv_widgets\\lv_canvas.c"
}
...
void lv_canvas_blur_hor(lv_obj_t * canvas, const lv_area_t * area, uint16_t r)
{
    ...
    _Bool has_alpha = lv_img_cf_has_alpha(ext->dsc.header.cf);
    ...
    for(y = a.y1; y <= a.y2; y++) {
        ...
        for(x = a.x1; x <= a.x2; x++) {
            ...
            x_safe = x + 1 + r_front;
            x_safe = x_safe > ext->dsc.header.w - 1 ? ext->dsc.header.w - 1 : x_safe;
            c = lv_img_buf_get_px_color(&line_img, x_safe, 0, ((lv_color_t){{(uint16_t)((0x00 >> 3) & 0x1FU), (uint16_t)((0x00 >> 2) & 0x3FU), (uint16_t)((0xFF >> 3) & 0x1FU)}}));
            if(has_alpha) opa = lv_img_buf_get_px_alpha(&line_img, x_safe, 0);
                         ...
                         const lv_draw_rect_dsc_t * rect_dsc)
{
    ...
    lv_disp_buf_init(&disp_buf, (void *)dsc->data, 0, dsc->header.w * dsc->header.h);
    ...
    lv_color_t ctransp = ((lv_color_t){{(uint16_t)((0x00 >> 3) & 0x1FU), (uint16_t)((0xFF >> 2) & 0x3FU), (uint16_t)((0x00 >> 3) & 0x1FU)}});
                         ...
                         const char * txt, lv_label_align_t align)
{
    ...
    lv_draw_label(&coords, &mask, label_draw_dsc, txt, 0);
...
static lv_res_t lv_canvas_signal(lv_obj_t * canvas, lv_signal_t sign, void * param)
{
    ...
    if(sign == LV_SIGNAL_GET_TYPE) return lv_obj_handle_get_type_signal(param, "lv_canvas");
                                    ...
                                    lv_color_t color, lv_opa_t opa)
{
    ...
    lv_color_t bg_color = lv_img_buf_get_px_color(&d, x, y, ((lv_color_t){{(uint16_t)((0x00 >> 3) & 0x1FU), (uint16_t)((0x00 >> 2) & 0x3FU), (uint16_t)((0x00 >> 3) & 0x1FU)}}));
