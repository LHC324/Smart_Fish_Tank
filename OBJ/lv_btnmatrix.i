#line 1 "..\\GUI\\lvgl_src\\lv_widgets\\lv_btnmatrix.c"
...
#line 10 "..\\GUI\\lvgl_src\\lv_widgets\\lv_btnmatrix.c"


#line 13 "..\\GUI\\lvgl_src\\lv_widgets\\lv_btnmatrix.c"
#line 14 "..\\GUI\\lvgl_src\\lv_widgets\\lv_btnmatrix.c"
#line 15 "..\\GUI\\lvgl_src\\lv_widgets\\lv_btnmatrix.c"
...
#line 16 "..\\GUI\\lvgl_src\\lv_widgets\\lv_btnmatrix.c"
...
#line 17 "..\\GUI\\lvgl_src\\lv_widgets\\lv_btnmatrix.c"
#line 18 "..\\GUI\\lvgl_src\\lv_widgets\\lv_btnmatrix.c"
...
static _Bool button_is_hidden(lv_btnmatrix_ctrl_t ctrl_bits);
static _Bool button_is_repeat_disabled(lv_btnmatrix_ctrl_t ctrl_bits);
static _Bool button_is_inactive(lv_btnmatrix_ctrl_t ctrl_bits);
static _Bool button_is_click_trig(lv_btnmatrix_ctrl_t ctrl_bits);
static _Bool button_is_tgl_enabled(lv_btnmatrix_ctrl_t ctrl_bits);
static _Bool button_get_tgl_state(lv_btnmatrix_ctrl_t ctrl_bits);
...
static _Bool maps_are_identical(const char ** map1, const char ** map2);
...
lv_obj_t * lv_btnmatrix_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    do { if(!((lv_debug_check_null(btnm)))) { ; lv_debug_log_error("Out of memory", (uint64_t)((uintptr_t)btnm)); while(1); } } while(0);;
    if(btnm == 0) return 0;

    if(ancestor_signal == 0) ancestor_signal = lv_obj_get_signal_cb(btnm);
    ...
    ext->btn_cnt        = 0;
    ext->btn_id_pr      = 0xFFFF;
    ext->btn_id_focused = 0xFFFF;
    ext->btn_id_act     = 0xFFFF;
    ext->button_areas   = 0;
    ext->ctrl_bits      = 0;
    ext->map_p          = 0;
    ext->recolor        = 0;
    ...
    if(ancestor_design_f == 0) ancestor_design_f = lv_obj_get_design_cb(btnm);
    ...
    if(copy == 0) {
        ...
        lv_obj_set_size(btnm, 60 * 2, 60 * 1);
...
void lv_btnmatrix_set_map(lv_obj_t * btnm, const char * map[])
{
    do { if(!((lv_debug_check_null(btnm)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)btnm)); while(1); } } while(0);;
    do { if(!((lv_debug_check_null(map)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)map)); while(1); } } while(0);;
...
void lv_btnmatrix_set_focused_btn(lv_obj_t * btnm, uint16_t id)
{
    ...
    if(id >= ext->btn_cnt && id != 0xFFFF) return;
...
void lv_btnmatrix_set_btn_width(lv_obj_t * btnm, uint16_t btn_id, uint8_t width)
{
    ...
    lv_btnmatrix_ext_t * ext = lv_obj_get_ext_attr(btnm);
    if(btn_id >= ext->btn_cnt) return;
    ext->ctrl_bits[btn_id] &= (~0x0007);
    ext->ctrl_bits[btn_id] |= (0x0007 & width);
...
const char * lv_btnmatrix_get_btn_text(const lv_obj_t * btnm, uint16_t btn_id)
{
    ...
    lv_btnmatrix_ext_t * ext = lv_obj_get_ext_attr(btnm);
    if(btn_id > ext->btn_cnt) return 0;
...
_Bool lv_btnmatrix_get_btn_ctrl(lv_obj_t * btnm, uint16_t btn_id, lv_btnmatrix_ctrl_t ctrl)
{
    ...
    return (ext->ctrl_bits[btn_id] & ctrl) ? 1 : 0;
}
...
static lv_design_res_t lv_btnmatrix_design(lv_obj_t * btnm, const lv_area_t * clip_area, lv_design_mode_t mode)
{
    if(mode == LV_DESIGN_COVER_CHK) {
        ...
        _lv_obj_disable_style_caching(btnm, 1);
        ...
        _lv_obj_disable_style_caching(btnm, 0);

        _Bool chk_inited = 0;
        _Bool disabled_inited = 0;
        ...
        for(btn_i = 0; btn_i < ext->btn_cnt; btn_i++, txt_i++) {
            ...
            lv_draw_label(&area_tmp, clip_area, draw_label_dsc_act, txt, 0);
        }
    }
...
static lv_res_t lv_btnmatrix_signal(lv_obj_t * btnm, lv_signal_t sign, void * param)
{
    ...
    if(sign == LV_SIGNAL_GET_TYPE) return lv_obj_handle_get_type_signal(param, "lv_btnmatrix");
    ...
    if(sign == LV_SIGNAL_CLEANUP) {
        ...
        if(indev_type == LV_INDEV_TYPE_POINTER || indev_type == LV_INDEV_TYPE_BUTTON) {
            ...
            if(btn_pr != 0xFFFF) {
                if(button_is_inactive(ext->ctrl_bits[btn_pr]) == 0 &&
                   button_is_hidden(ext->ctrl_bits[btn_pr]) == 0) {
                    invalidate_button_area(btnm, ext->btn_id_pr)  ;
        ...
        if(ext->btn_id_pr != 0xFFFF) {
            if(button_is_click_trig(ext->ctrl_bits[ext->btn_id_pr]) == 0 &&
               button_is_inactive(ext->ctrl_bits[ext->btn_id_pr]) == 0 &&
               button_is_hidden(ext->ctrl_bits[ext->btn_id_pr]) == 0) {
        ...
        if(btn_pr != ext->btn_id_pr) {
            ...
            if(btn_pr != 0xFFFF &&
               button_is_inactive(ext->ctrl_bits[btn_pr]) == 0 &&
               button_is_hidden(ext->ctrl_bits[btn_pr]) == 0) {
                invalidate_button_area(btnm, btn_pr);
                 
                if(button_is_click_trig(ext->ctrl_bits[btn_pr]) == 0) {
                    uint32_t b = btn_pr;
        ...
        if(ext->btn_id_pr != 0xFFFF) {
            ...
            if(button_is_click_trig(ext->ctrl_bits[ext->btn_id_act]) == 1 &&
               button_is_inactive(ext->ctrl_bits[ext->btn_id_act]) == 0 &&
               button_is_hidden(ext->ctrl_bits[ext->btn_id_act]) == 0) {
        ...
        if(ext->btn_id_act != 0xFFFF) {
            if(button_is_repeat_disabled(ext->ctrl_bits[ext->btn_id_act]) == 0 &&
               button_is_inactive(ext->ctrl_bits[ext->btn_id_act]) == 0 &&
               button_is_hidden(ext->ctrl_bits[ext->btn_id_act]) == 0) {
                uint32_t b = ext->btn_id_act;
                res        = lv_event_send(btnm, LV_EVENT_VALUE_CHANGED, &b);
        ...
        if(ext->btn_id_focused != 0xFFFF) invalidate_button_area(btnm, ext->btn_id_focused);
        if(ext->btn_id_pr != 0xFFFF) invalidate_button_area(btnm, ext->btn_id_pr);
        ext->btn_id_focused = 0xFFFF;
        ext->btn_id_pr = 0xFFFF;
        ext->btn_id_act = 0xFFFF;
    }
        ...
        if(c == LV_KEY_RIGHT) {
            ...
            if(ext->btn_id_focused == 0xFFFF) ext->btn_id_focused = 0;
            ...
            if(ext->btn_id_focused == 0xFFFF) {
                ...
                for(area_below = ext->btn_id_focused; area_below < ext->btn_cnt; area_below++) {
                    if(ext->button_areas[area_below].y1 > ext->button_areas[ext->btn_id_focused].y1 &&
                       ...
                       button_is_inactive(ext->ctrl_bits[area_below]) == 0 &&
                       button_is_hidden(ext->ctrl_bits[area_below]) == 0) {
                        break;
                    }
                }
            ...
            if(ext->btn_id_focused == 0xFFFF) {
                ...
                for(area_above = ext->btn_id_focused; area_above >= 0; area_above--) {
                    if(ext->button_areas[area_above].y1 < ext->button_areas[ext->btn_id_focused].y1 &&
                       ...
                       button_is_inactive(ext->ctrl_bits[area_above]) == 0 &&
                       button_is_hidden(ext->ctrl_bits[area_above]) == 0) {
                        break;
                    }
                }
...
static void allocate_btn_areas_and_controls(const lv_obj_t * btnm, const char ** map)
{
    ...
    do { if(!((lv_debug_check_null(ext->button_areas)))) { ; lv_debug_log_error("Out of memory", (uint64_t)((uintptr_t)ext->button_areas)); while(1); } } while(0);;
    ext->ctrl_bits = lv_mem_alloc(sizeof(lv_btnmatrix_ctrl_t) * btn_cnt);
    do { if(!((lv_debug_check_null(ext->ctrl_bits)))) { ; lv_debug_log_error("Out of memory", (uint64_t)((uintptr_t)ext->ctrl_bits)); while(1); } } while(0);;
    if(ext->button_areas == 0 || ext->ctrl_bits == 0) btn_cnt = 0;
...
static _Bool button_get_tgl_state(lv_btnmatrix_ctrl_t ctrl_bits)
{
    return (ctrl_bits & LV_BTNMATRIX_CTRL_CHECK_STATE) ? 1 : 0;
}
...
static uint16_t get_button_from_point(lv_obj_t * btnm, lv_point_t * p)
{
    ...
    pinner = ((pinner) < ((60 / 4)) ? (pinner) : ((60 / 4)));
    pright = ((pright) < ((60 / 4)) ? (pright) : ((60 / 4)));
    ptop = ((ptop) < ((60 / 4)) ? (ptop) : ((60 / 4)));
    pbottom = ((pbottom) < ((60 / 4)) ? (pbottom) : ((60 / 4)));

    for(i = 0; i < ext->btn_cnt; i++) {
        lv_area_copy(&btn_area, &ext->button_areas[i]);
        if(btn_area.x1 <= pleft) btn_area.x1 += btnm_cords.x1 - ((pleft) < ((60 / 4)) ? (pleft) : ((60 / 4)));
        else btn_area.x1 += btnm_cords.x1 - pinner;

        if(btn_area.y1 <= ptop) btn_area.y1 += btnm_cords.y1 - ((ptop) < ((60 / 4)) ? (ptop) : ((60 / 4)));
        else btn_area.y1 += btnm_cords.y1 - pinner;

        if(btn_area.x2 >= w - pright - 2) btn_area.x2 += btnm_cords.x1 + ((pright) < ((60 / 4)) ? (pright) : ((60 / 4)));   

        else btn_area.x2 += btnm_cords.x1 + pinner;

        if(btn_area.y2 >= h - pbottom - 2) btn_area.y2 += btnm_cords.y1 + ((pbottom) < ((60 / 4)) ? (pbottom) : ((60 / 4)));  
...
static _Bool maps_are_identical(const char ** map1, const char ** map2)
{
    ...
    if(map1 == 0 || map2 == 0) return map1 == map2;

    uint16_t i = 0;
    while(map1[i][0] != '\0' && map2[i][0] != '\0') {
        if(strcmp(map1[i], map2[i]) != 0) return 0;
        i++;
    }
...
static void make_one_button_toggled(lv_obj_t * btnm, uint16_t btn_idx)
{
    ...
    _Bool was_toggled = lv_btnmatrix_get_btn_ctrl(btnm, btn_idx, LV_BTNMATRIX_CTRL_CHECK_STATE);
