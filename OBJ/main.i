...
protocolTime_t *GET_NTPTime; 
volatile short temperature=0;
volatile short Distance=0;
volatile uint32_t random;

uint8_t TasK_Buffer[500]; 
uint8_t Cpu_Buffer[400]; 

 

QueueHandle_t Flag_Message_Queue; 

sideparm side_temperature ={0,{0.0},&side_temperature.sidebuff[0],&side_temperature.sidebuff[0],0.0}; 
...
TimerHandle_t AutoReloadTimer1_Handler,AutoReloadTimer2_Handler; 
...
void start_task(void * pvParameters){
	...
	BinarySemaphore=xQueueGenericCreate( ( UBaseType_t ) 1, ( ( uint8_t ) 0U ), ( ( uint8_t ) 3U ) );  
	if(BinarySemaphore!=0){
		xQueueGenericSend( ( QueueHandle_t ) ( BinarySemaphore ), 0, ( ( TickType_t ) 0U ), ( ( BaseType_t ) 0 ) ); 
	}
	
	 
	xTaskCreate((TaskFunction_t	) lcd_task,
				(const char*	) "lcd_task",
				(const TickType_t) 512,
				(void * 		) 0,
				(UBaseType_t	) 3,
				(TaskHandle_t*	) &LcdTask_Handler);
	...
	AutoReloadTimer1_Handler=(TimerHandle_t)xTimerCreate(
				(const char *     ) "time1_task",
                (const TickType_t ) 1000, 
                (const UBaseType_t) ( ( BaseType_t ) 1 ),
                (void *           ) 1,
                (TimerCallbackFunction_t) AutoReloadCallback);
				
	 			
	AutoReloadTimer2_Handler=(TimerHandle_t)xTimerCreate(
				(const char *     ) "time2_task",
                (const TickType_t ) 60, 
                (const UBaseType_t) ( ( BaseType_t ) 1 ),
                (void *           ) 10,
                (TimerCallbackFunction_t) AutoReloadCallback);
	xTimerGenericCommand( ( AutoReloadTimer1_Handler ), ( ( BaseType_t ) 1 ), ( xTaskGetTickCount() ), 0, ( 0 ) );
	xTimerGenericCommand( ( AutoReloadTimer2_Handler ), ( ( BaseType_t ) 1 ), ( xTaskGetTickCount() ), 0, ( 0 ) );
...
void lcd_task(void * pvParameters){
	while(1){	
		...
		LCD_ShowxNum(148,140,temperature/10,2,16,0x00);	
		LCD_ShowChar(164,140,'.',16,0);	
   		LCD_ShowxNum(172,140,temperature%10,1,16,0x00);	
...
void event_task(void *parameter){
	while(1){
		...
		{ memset(TasK_Buffer,0,sizeof(TasK_Buffer)); vTaskList((char *)&TasK_Buffer); Usart1_DmaPrintf("任务名      任务状态  优先级   剩余栈  任务序号\r\n"); Usart1_DmaPrintf("%s\r\n", TasK_Buffer); }; 
...
void collect_task(void * pvParameters){
	while(1){ 
		...
		HAL_RTC_GetTime(&RTC_Handler,&RTC_TimeStruct,0x00000000U); 
        HAL_RTC_GetDate(&RTC_Handler,&RTC_DateStruct,0x00000000U);
		
		temperature = sidefilter((float)DS18B20_Get_Temperature(),&side_temperature);
        currentDataPoint.valueDS18B20 = temperature/10.0;



		*((volatile unsigned long *)((((((0x40000000UL + 0x00020000UL) + 0x0000UL)+20) & 0xF0000000)+0x2000000+(((((0x40000000UL + 0x00020000UL) + 0x0000UL)+20) &0xFFFFF)<<5)+(6<<2)))) =! *((volatile unsigned long *)((((((0x40000000UL + 0x00020000UL) + 0x0000UL)+20) & 0xF0000000)+0x2000000+(((((0x40000000UL + 0x00020000UL) + 0x0000UL)+20) &0xFFFFF)<<5)+(6<<2)))); 
		vTaskDelay(100);
	}
}
...
static _Bool update_time=0; 
	while(1){
		...
		if(UART3IDLE_RxOverFlag){
			...
			HAL_UART_Receive_DMA(&UART3_Handler,USART3_RX_BUF,512);	
		}
		
		
		tp_dev.scan(0); 
		gizwitsHandle((dataPoint_t *)&currentDataPoint); 
		if((wifi_success_flag) && (update_time==0)){ 
			gizwitsGetNTP(); 
			update_time=1; 
		}
		vTaskDelay(5);
...
void AutoReloadCallback(TimerHandle_t xTimer){
	if(xTimer==AutoReloadTimer1_Handler){

	}
	else if(xTimer == AutoReloadTimer2_Handler){
		if(BinarySemaphore !=0){
			Distance = sidefilter((float)Hcsr04_Get_Length()*100,&side_waterlevel);
			currentDataPoint.valueWater_level = (currentDataPoint.valueDefault_Water_Max+4.0f)-Distance/100.0;
			xQueueGenericSend( ( QueueHandle_t ) ( BinarySemaphore ), 0, ( ( TickType_t ) 0U ), ( ( BaseType_t ) 0 ) ); 
		}
	}
}
...
void Init_All(void){ 
	...
	gizwitsInit(); 
	keyInit();
	null_founction("MCU Init Success \r\n");
	...
	LCD_Clear(0x0000);
	POINT_COLOR = 0x0000;
	BACK_COLOR = RGB888toRGB565(150, 255, 255);
	...
	while(AT24CXX_Check()){ 
		...
		delay_ms(1000);
		LCD_Fill(30,70,200+30,70+16,0x0000);
		delay_ms(200);
		break;
	}
...
void led_transaction(void){ 
	if((auto_flag & 0x01) == 0x01){ 
		random=RNG_Get_RandomRange(0,16777215u);
...
void temperature_transaction(void){ 
	if((auto_flag & 0x02) == 0x02){ 
		if(currentDataPoint.valueDS18B20 < currentDataPoint.valueTemperature_Min){ 
			currentDataPoint.valueTemperature_Alarm = 0x01; 
			 
			((1 == 1) ? (HAL_GPIO_WritePin(((GPIO_TypeDef *) ((0x40000000UL + 0x00020000UL) + 0x0800UL)),((uint16_t)0x0001),GPIO_PIN_SET)): ((HAL_GPIO_WritePin(((GPIO_TypeDef *) ((0x40000000UL + 0x00020000UL) + 0x0800UL)),((uint16_t)0x0001),GPIO_PIN_RESET))));
			currentDataPoint.valueAdd_Temperature = 0x01; 
		}
		else if(currentDataPoint.valueDS18B20 > currentDataPoint.valueTemperature_Max){ 
			currentDataPoint.valueDS18B20_Malfunction = 0x01; 
			 
			((1 == 1) ? (HAL_GPIO_WritePin(((GPIO_TypeDef *) ((0x40000000UL + 0x00020000UL) + 0x0800UL)),((uint16_t)0x0002),GPIO_PIN_SET)): ((HAL_GPIO_WritePin(((GPIO_TypeDef *) ((0x40000000UL + 0x00020000UL) + 0x0800UL)),((uint16_t)0x0002),GPIO_PIN_RESET))));
		...
		if(currentDataPoint.valueDS18B20 < currentDataPoint.valueTemperature_Min || 		((currentDataPoint.valueDS18B20 >= currentDataPoint.valueTemperature_Min) && 		(currentDataPoint.valueDS18B20 <= currentDataPoint.valueTemperature_Max))){
			...
			((0 == 1) ? (HAL_GPIO_WritePin(((GPIO_TypeDef *) ((0x40000000UL + 0x00020000UL) + 0x0800UL)),((uint16_t)0x0002),GPIO_PIN_SET)): ((HAL_GPIO_WritePin(((GPIO_TypeDef *) ((0x40000000UL + 0x00020000UL) + 0x0800UL)),((uint16_t)0x0002),GPIO_PIN_RESET))));
			...
			((0 == 1) ? (HAL_GPIO_WritePin(((GPIO_TypeDef *) ((0x40000000UL + 0x00020000UL) + 0x0800UL)),((uint16_t)0x0001),GPIO_PIN_SET)): ((HAL_GPIO_WritePin(((GPIO_TypeDef *) ((0x40000000UL + 0x00020000UL) + 0x0800UL)),((uint16_t)0x0001),GPIO_PIN_RESET))));
			currentDataPoint.valueAdd_Temperature = 0x00; 
			currentDataPoint.valueDS18B20_Malfunction = 0x01; 
...
void water_level_transaction(void){ 
	if((auto_flag & 0x08) == 0x08){ 
		
		if(currentDataPoint.valueWater_level < currentDataPoint.valueDefault_Water_Min){ 
			currentDataPoint.valueWater_Level_Alarm = 0x01; 
			 
			((1 == 1) ? (HAL_GPIO_WritePin(((GPIO_TypeDef *) ((0x40000000UL + 0x00020000UL) + 0x0800UL)),((uint16_t)0x0004),GPIO_PIN_SET)): ((HAL_GPIO_WritePin(((GPIO_TypeDef *) ((0x40000000UL + 0x00020000UL) + 0x0800UL)),((uint16_t)0x0004),GPIO_PIN_RESET))));
			currentDataPoint.valuePump = 0x01; 
		}
		else if(currentDataPoint.valueWater_level > currentDataPoint.valueDefault_Water_Max){ 
			currentDataPoint.valueWater_Level_Malfunction = 0x01; 
			 
			((0 == 1) ? (HAL_GPIO_WritePin(((GPIO_TypeDef *) ((0x40000000UL + 0x00020000UL) + 0x0800UL)),((uint16_t)0x0004),GPIO_PIN_SET)): ((HAL_GPIO_WritePin(((GPIO_TypeDef *) ((0x40000000UL + 0x00020000UL) + 0x0800UL)),((uint16_t)0x0004),GPIO_PIN_RESET))));
		...
		if(currentDataPoint.valueWater_level < currentDataPoint.valueDefault_Water_Min || 		((currentDataPoint.valueWater_level >= currentDataPoint.valueDefault_Water_Min) && 		(currentDataPoint.valueWater_level <= currentDataPoint.valueDefault_Water_Max))){
	...
	if((currentDataPoint.valueWater_level >= currentDataPoint.valueDefault_Water_Min) && 		(currentDataPoint.valueWater_level <= currentDataPoint.valueDefault_Water_Max)){ 
...
void oxygen_transaction(void){ 
static _Bool state_mutex = 1; 
	static _Bool help_flag = 1;
	static _Bool lock = 1;

	if((auto_flag & 0x04) == 0x04){ 
		allow_flag &= 0xFB; 
		if(state_mutex == 1){
			if(lock == 1){
				lock = 0;
				...
				((1 == 1) ? (HAL_GPIO_WritePin(((GPIO_TypeDef *) ((0x40000000UL + 0x00020000UL) + 0x0800UL)),((uint16_t)0x0008),GPIO_PIN_SET)): ((HAL_GPIO_WritePin(((GPIO_TypeDef *) ((0x40000000UL + 0x00020000UL) + 0x0800UL)),((uint16_t)0x0008),GPIO_PIN_RESET))));
				currentDataPoint.valueOxygen_Pump = 0x01; 
			}
		}
		else{
			if(lock == 1){
				...
				((0 == 1) ? (HAL_GPIO_WritePin(((GPIO_TypeDef *) ((0x40000000UL + 0x00020000UL) + 0x0800UL)),((uint16_t)0x0008),GPIO_PIN_SET)): ((HAL_GPIO_WritePin(((GPIO_TypeDef *) ((0x40000000UL + 0x00020000UL) + 0x0800UL)),((uint16_t)0x0008),GPIO_PIN_RESET))));
				currentDataPoint.valueOxygen_Pump = 0x00; 
			}
		}
		
		if((RTC_TimeStruct.Hours == nextTime.nexthour) && (RTC_TimeStruct.Minutes == nextTime.nextmin) && (help_flag == 1)){
			help_flag =0;
			lock = 1;
	...
	if((auto_flag & 0x10) == 0x10){ 
		...
		if((RTC_TimeStruct.Hours == nextTime1.nexthour) && (RTC_TimeStruct.Minutes == nextTime1.nextmin)){
			if(count > 0){
				...
				servo_angle(5); 
				currentDataPoint.valueServo = Servo_VALUE4;
				count--;
			}
		}
		else{ 
			...
			servo_angle(0);
			currentDataPoint.valueServo = Servo_VALUE1;
		}
...
void get_FeedEndTime(void){
	...
	if(first_flag == 0){ 
		nextTime1.nexthour =  currentDataPoint.valueFeed_Hour; 
...
int ConfigureTimerForRunTimeStats(void){ 

	FreeRTOSRunTimeTicks = 0; 
	TIM7_Int_Init(50-1,84-1);       
	return 0;
}

void FreeRtos_CPU_state_Debug(void){
	memset(Cpu_Buffer,0,sizeof(Cpu_Buffer)); 				
	vTaskGetRunTimeStats((char*)Cpu_Buffer);  	
