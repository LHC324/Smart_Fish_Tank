#line 1 "..\\GUI\\lvgl_src\\lv_widgets\\lv_chart.c"
...
#line 10 "..\\GUI\\lvgl_src\\lv_widgets\\lv_chart.c"


#line 13 "..\\GUI\\lvgl_src\\lv_widgets\\lv_chart.c"
...
#line 14 "..\\GUI\\lvgl_src\\lv_widgets\\lv_chart.c"
...
#line 15 "..\\GUI\\lvgl_src\\lv_widgets\\lv_chart.c"
#line 16 "..\\GUI\\lvgl_src\\lv_widgets\\lv_chart.c"
...
#line 17 "..\\GUI\\lvgl_src\\lv_widgets\\lv_chart.c"
...
#line 32 "..\\GUI\\lvgl_src\\lv_widgets\\lv_chart.c"
...
static inline _Bool is_tick_with_label(uint8_t tick_num, lv_chart_axis_cfg_t * axis);
...
lv_obj_t * lv_chart_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    do { if(!((lv_debug_check_null(chart)))) { ; lv_debug_log_error("Out of memory", (uint64_t)((uintptr_t)chart)); while(1); } } while(0);;
    if(chart == 0) return 0;
    ...
    ext->x_axis.major_tick_len = 255;
    ext->x_axis.minor_tick_len = 255;
    ext->y_axis.major_tick_len = 255;
    ext->y_axis.minor_tick_len = 255;
    ext->secondary_y_axis.major_tick_len = 255;
    ext->secondary_y_axis.minor_tick_len = 255;

    lv_style_list_init(&ext->style_series_bg);
    lv_style_list_init(&ext->style_series);
    lv_style_list_init(&ext->style_cursors);

    if(ancestor_design == 0) ancestor_design = lv_obj_get_design_cb(chart);
    if(ancestor_signal == 0) ancestor_signal = lv_obj_get_signal_cb(chart);
    ...
    if(copy == 0) {
        lv_obj_set_size(chart, 60 * 3, 60 * 2);
        ...
        lv_obj_refresh_style(chart, LV_OBJ_PART_ALL, 0xFF);
    }
...
lv_chart_series_t * lv_chart_add_series(lv_obj_t * chart, lv_color_t color)
{
    do { if(!((lv_debug_check_null(chart)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)chart)); while(1); } } while(0);;

    lv_chart_ext_t * ext    = lv_obj_get_ext_attr(chart);
    lv_chart_series_t * ser = _lv_ll_ins_head(&ext->series_ll);
    do { if(!((lv_debug_check_null(ser)))) { ; lv_debug_log_error("Out of memory", (uint64_t)((uintptr_t)ser)); while(1); } } while(0);;
    if(ser == 0) return 0;

    lv_coord_t def = ((-((lv_coord_t)((uint32_t)((uint32_t)1 << (8 * sizeof(lv_coord_t) - 1)) - 1000))));

    ser->color  = color;
    ser->points = lv_mem_alloc(sizeof(lv_coord_t) * ext->point_cnt);
    do { if(!((lv_debug_check_null(ser->points)))) { ; lv_debug_log_error("Out of memory", (uint64_t)((uintptr_t)ser->points)); while(1); } } while(0);;
    if(ser->points == 0) {
...
lv_chart_cursor_t  * lv_chart_add_cursor(lv_obj_t * chart, lv_color_t color, lv_cursor_direction_t axes)
{
    ...
    do { if(!((lv_debug_check_null(cursor)))) { ; lv_debug_log_error("Out of memory", (uint64_t)((uintptr_t)cursor)); while(1); } } while(0);;
    if(cursor == 0) return 0;

    cursor->point.x = 0U;
    cursor->point.y = ((-((lv_coord_t)((uint32_t)((uint32_t)1 << (8 * sizeof(lv_coord_t) - 1)) - 1000))));
    cursor->color = color;
    cursor->axes = axes;
...
{
    do { if(!((lv_debug_check_null(chart)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)chart)); while(1); } } while(0);;
    do { if(!((lv_debug_check_null(series)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)series)); while(1); } } while(0);;

    if(chart == 0 || series == 0) return;
    lv_chart_ext_t * ext = lv_obj_get_ext_attr(chart);
    if(ext == 0) return;

    uint32_t i;
    for(i = 0; i < ext->point_cnt; i++) {
        series->points[i] = ((-((lv_coord_t)((uint32_t)((uint32_t)1 << (8 * sizeof(lv_coord_t) - 1)) - 1000))));
    }
...
void lv_chart_set_point_count(lv_obj_t * chart, uint16_t point_cnt)
{
    ...
    for(ser = _lv_ll_get_tail(&ext->series_ll); ser != 0; ser = _lv_ll_get_prev(&ext->series_ll, ser)) {
        if(!ser->ext_buf_assigned) {
            if(ser->start_point != 0) {
                lv_coord_t * new_points = lv_mem_alloc(sizeof(lv_coord_t) * point_cnt);
                do { if(!((lv_debug_check_null(new_points)))) { ; lv_debug_log_error("Out of memory", (uint64_t)((uintptr_t)new_points)); while(1); } } while(0);;
...
{
    do { if(!((lv_debug_check_null(chart)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)chart)); while(1); } } while(0);;
    do { if(!((lv_debug_check_null(ser)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)ser)); while(1); } } while(0);;
...
{
    do { if(!((lv_debug_check_null(chart)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)chart)); while(1); } } while(0);;
    do { if(!((lv_debug_check_null(list_of_values)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)list_of_values)); while(1); } } while(0);;
...
void lv_chart_set_cursor_point(lv_obj_t * chart, lv_chart_cursor_t * cursor, lv_point_t * point)
{
	do { if(!((lv_debug_check_null(cursor)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)cursor)); while(1); } } while(0);;
	((void) chart);
...
lv_coord_t lv_chart_get_x_from_index(lv_obj_t * chart, lv_chart_series_t * ser, uint16_t id)
{
	...
	if(ext->type & LV_CHART_TYPE_LINE) {
		...
		lv_coord_t col_w = w / ((_lv_ll_get_len(&ext->series_ll) + 1) * ext->point_cnt);  
		lv_chart_series_t * itr_ser = 0;
		lv_style_int_t col_space = lv_obj_get_style_pad_inner(chart, LV_CHART_PART_SERIES);

		x = (int32_t)((int32_t)w * id) / ext->point_cnt;
		x += col_w / 2;  

		for(itr_ser = _lv_ll_get_tail(&ext->series_ll); itr_ser != 0; itr_ser = _lv_ll_get_prev(&ext->series_ll, itr_ser)) {
...
static lv_res_t lv_chart_signal(lv_obj_t * chart, lv_signal_t sign, void * param)
{
    ...
    if(sign == LV_SIGNAL_GET_TYPE) return lv_obj_handle_get_type_signal(param, "lv_chart");

    lv_chart_ext_t * ext = lv_obj_get_ext_attr(chart);

    if(sign == LV_SIGNAL_CLEANUP) {
        lv_chart_series_t * ser;
        while(ext->series_ll.head != 0) {
...
{
    lv_area_t com_area;
    if(_lv_area_intersect(&com_area, series_area, clip_area) == 0) return;
    ...
    _Bool mask_ret = _lv_area_intersect(&series_mask, series_area, clip_area);
    ...
    int16_t mask_fade_id = (-1);
    lv_draw_rect_dsc_t area_dsc;
    _Bool has_area = lv_obj_get_style_bg_opa(chart, LV_CHART_PART_SERIES) > 2 ? 1 : 0;
    _Bool has_fade = 0;
    if(has_area) {
        ...
        has_fade = area_dsc.bg_grad_dir == LV_GRAD_DIR_VER ? 1 : 0;
        if(has_fade) {
    ...
    for(ser = _lv_ll_get_tail(&ext->series_ll); ser != 0; ser = _lv_ll_get_prev(&ext->series_ll, ser)) {
        ...
        for(i = 0; i < ext->point_cnt; i++) {
            ...
            if(i != 0 && ser->points[p_prev] != ((-((lv_coord_t)((uint32_t)((uint32_t)1 << (8 * sizeof(lv_coord_t) - 1)) - 1000)))) && ser->points[p_act] != ((-((lv_coord_t)((uint32_t)((uint32_t)1 << (8 * sizeof(lv_coord_t) - 1)) - 1000))))) {
                lv_draw_line(&p1, &p2, &series_mask, &line_dsc);

                lv_coord_t y_top = ((p1 . y) < (p2 . y) ? (p1 . y) : (p2 . y));
                if(has_area && y_top <= clip_area->y2) {
                    ...
                    mask_line_id = lv_draw_mask_add(&mask_line_p, 0);
                    ...
                    if(has_fade) mask_fade_id = lv_draw_mask_add(&mask_fade_p, 0);
            ...
            if(point_radius) {
                ...
                if(ser->points[p_act] != ((-((lv_coord_t)((uint32_t)((uint32_t)1 << (8 * sizeof(lv_coord_t) - 1)) - 1000))))) {
...
static void draw_cursors(lv_obj_t * chart, const lv_area_t * series_area, const lv_area_t * clip_area)
{
    ...
    for(cursor = _lv_ll_get_tail(&ext->cursors_ll); cursor != 0; cursor = _lv_ll_get_prev(&ext->cursors_ll, cursor)) {
...
static void draw_y_ticks(lv_obj_t * chart, const lv_area_t * series_area, const lv_area_t * mask, uint8_t which_axis)
{
    ...
    if(y_axis->list_of_values == 0 && y_axis->num_tick_marks == 0)  return;
    ...
    if(y_axis->major_tick_len == 255)
        major_tick_len = (int32_t)w * 1 / 15;
    else
        major_tick_len = y_axis->major_tick_len;

    if(y_axis->minor_tick_len == 255)
    ...
    iter_dir = (y_axis->options & LV_CHART_AXIS_INVERSE_LABELS_ORDER) ? 0 :
               1;
        ...
        if(num_of_labels != 0) {
            ...
            if(is_tick_with_label(i, y_axis)) {
                ...
                _lv_txt_get_size(&size, buf, label_dsc.font, label_dsc.letter_space, label_dsc.line_space,
                                 ((lv_coord_t)((uint32_t)((uint32_t)1 << (8 * sizeof(lv_coord_t) - 1)) - 1000)), LV_TXT_FLAG_CENTER);
                ...
                lv_draw_label(&a, mask, &label_dsc, buf, 0);
            }
        }
    }
}
...
static void draw_x_ticks(lv_obj_t * chart, const lv_area_t * series_area, const lv_area_t * mask)
{
    ...
    if(ext->x_axis.list_of_values == 0 && ext->x_axis.num_tick_marks == 0) return;
    ...
    if(ext->x_axis.major_tick_len == 255)
        major_tick_len = (int32_t)w * 1 / 15;
    else
        major_tick_len = ext->x_axis.major_tick_len;

    if(ext->x_axis.minor_tick_len == 255)
        minor_tick_len = major_tick_len * 2 / 3;
    else
    ...
    if(ext->type == LV_CHART_TYPE_COLUMN) {
        ...
        lv_coord_t col_w = w / ((ser_num + 1) * ext->point_cnt);  
    ...
    iter = create_axis_label_iter(ext->x_axis.list_of_values, 1);
    num_of_labels = iter.items_left;
