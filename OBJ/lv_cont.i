#line 1 "..\\GUI\\lvgl_src\\lv_widgets\\lv_cont.c"
...
#line 11 "..\\GUI\\lvgl_src\\lv_widgets\\lv_cont.c"


#line 14 "..\\GUI\\lvgl_src\\lv_widgets\\lv_cont.c"
#line 15 "..\\GUI\\lvgl_src\\lv_widgets\\lv_cont.c"
#line 16 "..\\GUI\\lvgl_src\\lv_widgets\\lv_cont.c"

#line 18 "..\\GUI\\lvgl_src\\lv_widgets\\lv_cont.c"
...
#line 19 "..\\GUI\\lvgl_src\\lv_widgets\\lv_cont.c"
#line 20 "..\\GUI\\lvgl_src\\lv_widgets\\lv_cont.c"
...
#line 21 "..\\GUI\\lvgl_src\\lv_widgets\\lv_cont.c"
#line 22 "..\\GUI\\lvgl_src\\lv_widgets\\lv_cont.c"
#line 23 "..\\GUI\\lvgl_src\\lv_widgets\\lv_cont.c"
#line 24 "..\\GUI\\lvgl_src\\lv_widgets\\lv_cont.c"
...
lv_obj_t * lv_cont_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    do { if(!((lv_debug_check_null(cont)))) { ; lv_debug_log_error("Out of memory", (uint64_t)((uintptr_t)cont)); while(1); } } while(0);;
    if(cont == 0) return 0;

    if(ancestor_signal == 0) ancestor_signal = lv_obj_get_signal_cb(cont);
    if(ancestor_design == 0) ancestor_design = lv_obj_get_design_cb(cont);
    ...
    if(copy == 0) {
        ...
        lv_obj_refresh_style(cont, LV_OBJ_PART_ALL, 0xFF);
    }
...
void lv_cont_set_layout(lv_obj_t * cont, lv_layout_t layout)
{
    do { if(!((lv_debug_check_null(cont)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)cont)); while(1); } } while(0);;
    ...
    cont->signal_cb(cont, LV_SIGNAL_CHILD_CHG, 0);
}
...
static lv_res_t lv_cont_signal(lv_obj_t * cont, lv_signal_t sign, void * param)
{
    ...
    if(sign == LV_SIGNAL_GET_TYPE) return lv_obj_handle_get_type_signal(param, "lv_cont");
...
static void lv_cont_refr_layout(lv_obj_t * cont)
{
    ...
    if(lv_obj_get_child(cont, 0) == 0) return;
...
static void lv_cont_layout_col(lv_obj_t * cont)
{
    ...
    for(child = _lv_ll_get_tail(&cont->child_ll); child != 0; child = _lv_ll_get_prev(&cont->child_ll, child)) {
        if(lv_obj_get_hidden(child) != 0 || lv_obj_is_protected(child, LV_PROTECT_POS) != 0) continue;
...
static void lv_cont_layout_center(lv_obj_t * cont)
{
    ...
    lv_coord_t inner = lv_obj_get_style_pad_inner(cont, LV_CONT_PART_MAIN);
    for(child = _lv_ll_get_head(&cont->child_ll); child != 0; child = _lv_ll_get_next(&cont->child_ll, child)) {
        if(lv_obj_get_hidden(child) != 0 || lv_obj_is_protected(child, LV_PROTECT_POS) != 0) continue;
...
static void lv_cont_layout_pretty(lv_obj_t * cont)
{
    ...
    child_rs = _lv_ll_get_tail(&cont->child_ll);  
    if(child_rs == 0) return;                 
    ...
    while(child_rs != 0) {
        lv_coord_t h_row = 0;
        lv_coord_t w_row = pleft + pright;  
        uint32_t obj_num = 0;

         
        do {
            if(lv_obj_get_hidden(child_rc) == 0 && lv_obj_is_protected(child_rc, LV_PROTECT_POS) == 0) {
                ...
                h_row = ((h_row) > (h) ? (h_row) : (h));          
                obj_num++;
                if(lv_obj_is_protected(child_rc, LV_PROTECT_FOLLOW))
                    break;  
            }
            child_rc = _lv_ll_get_prev(&cont->child_ll, child_rc);  
            if(obj_num == 0)
                child_rs = child_rc; 
        ...
        if(obj_num == 0) {
            ...
            while(child_tmp != 0) {
                if(lv_obj_get_hidden(child_tmp) == 0 && lv_obj_is_protected(child_tmp, LV_PROTECT_POS) == 0) {
        ...
        if(child_rc == 0) break;
        act_y += pinner + h_row;            
        child_rs = _lv_ll_get_prev(&cont->child_ll, child_rc);  
        child_rc = child_rs;
    }
    lv_obj_clear_protect(cont, LV_PROTECT_CHILD_CHG);
}
...
static void lv_cont_layout_grid(lv_obj_t * cont)
{
    ...
    lv_coord_t inner = lv_obj_get_style_pad_inner(cont, LV_CONT_PART_MAIN);
    lv_coord_t y_ofs = inner + lv_obj_get_height(lv_obj_get_child(cont, 0));
...
static void lv_cont_refr_autofit(lv_obj_t * cont)
{
    ...
    _Bool has_children = _lv_ll_is_empty(&cont->child_ll) ? 0 : 1;

    if(has_children) {
        tight_area.x1 = ((lv_coord_t)((uint32_t)((uint32_t)1 << (8 * sizeof(lv_coord_t) - 1)) - 1000));
        tight_area.y1 = ((lv_coord_t)((uint32_t)((uint32_t)1 << (8 * sizeof(lv_coord_t) - 1)) - 1000));
        tight_area.x2 = (-((lv_coord_t)((uint32_t)((uint32_t)1 << (8 * sizeof(lv_coord_t) - 1)) - 1000)));
        tight_area.y2 = (-((lv_coord_t)((uint32_t)((uint32_t)1 << (8 * sizeof(lv_coord_t) - 1)) - 1000)));

        for(child_i = _lv_ll_get_head(&cont->child_ll); child_i != 0; child_i = _lv_ll_get_next(&cont->child_ll, child_i)) {
            if(lv_obj_get_hidden(child_i) != 0) continue;

            if(ext->fit_left != LV_FIT_PARENT) {
                lv_style_int_t mleft = lv_obj_get_style_margin_left(child_i, LV_OBJ_PART_MAIN);
                tight_area.x1 = ((tight_area . x1) < (child_i->coords . x1 - mleft) ? (tight_area . x1) : (child_i->coords . x1 - mleft));
            }

            if(ext->fit_right != LV_FIT_PARENT) {
                lv_style_int_t mright = lv_obj_get_style_margin_right(child_i, LV_OBJ_PART_MAIN);
                tight_area.x2 = ((tight_area . x2) > (child_i->coords . x2 + mright) ? (tight_area . x2) : (child_i->coords . x2 + mright));
            }

            if(ext->fit_top != LV_FIT_PARENT) {
                lv_style_int_t mtop = lv_obj_get_style_margin_top(child_i, LV_OBJ_PART_MAIN);
                tight_area.y1 = ((tight_area . y1) < (child_i->coords . y1 - mtop) ? (tight_area . y1) : (child_i->coords . y1 - mtop));
            }

            if(ext->fit_bottom != LV_FIT_PARENT) {
                lv_style_int_t mbottom = lv_obj_get_style_margin_bottom(child_i, LV_OBJ_PART_MAIN);
                tight_area.y2 = ((tight_area . y2) > (child_i->coords . y2 + mbottom) ? (tight_area . y2) : (child_i->coords . y2 + mbottom));
            }
        }
    ...
    switch(ext->fit_left) {
        ...
        case LV_FIT_PARENT:
            ...
            break;
        case LV_FIT_MAX:
            new_area.x1 = has_children ? ((tight_area . x1) < (parent_area . x1) ? (tight_area . x1) : (parent_area . x1)) : parent_area.x1;
            break;
        default:
            break;
    }
    ...
    switch(ext->fit_right) {
        ...
        case LV_FIT_PARENT:
            ...
            break;
        case LV_FIT_MAX:
            new_area.x2 = has_children ? ((tight_area . x2) > (parent_area . x2) ? (tight_area . x2) : (parent_area . x2)) : parent_area.x2;
            break;
        default:
            break;
    }
    ...
    switch(ext->fit_top) {
        ...
        case LV_FIT_PARENT:
            ...
            break;
        case LV_FIT_MAX:
            new_area.y1 = has_children ? ((tight_area . y1) < (parent_area . y1) ? (tight_area . y1) : (parent_area . y1)) : parent_area.y1;
            break;
        default:
            break;
    }
    ...
    switch(ext->fit_bottom) {
        ...
        case LV_FIT_PARENT:
            ...
            break;
        case LV_FIT_MAX:
            new_area.y2 = has_children ? ((tight_area . y2) > (parent_area . y2) ? (tight_area . y2) : (parent_area . y2)) : parent_area.y2;
            break;
        default:
            break;
    }
