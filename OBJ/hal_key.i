...
#line 21 "..\\HARDWARE\\KEY\\hal_key.c"
...
#line 22 "..\\HARDWARE\\KEY\\hal_key.c"
...
uint16_t getKey(keysTypedef_t *keyS)
{
    ...
    for(i = 0; i < keys.keyTotolNum; i++)
    {
        ...
        {
            (readKey |= (1 << keyS->singleKey[i]. keyNum));
        }
    }
        ...
        switch (keyState)
        {
            ...
            case 2:
                ...
                if(keyPress != keyPrev)
                {
                    keyState = 0;
                    keyLongCheck = 0;
                    keyReturn = keyPrev | 0x2000;
                    return keyReturn;
                }
                if(keyPress == keyPrev)
                {
                    keyLongCheck++;
                    if(keyLongCheck >= (3000 / 30))    
                    {
                        keyLongCheck = 0;
                        keyState = 3;
                        keyReturn= keyPress | 0x8000;
                        return keyReturn;
                    }
                }
                break;
...
void keyHandle(keysTypedef_t *keyS)
{
    ...
    if(key_value & 0x2000)
    {
        ...
        for(i = 0; i < keyS->keyTotolNum; i++)
        {
            if((key_value & (1 << keyS->singleKey[i]. keyNum))) 
            {
