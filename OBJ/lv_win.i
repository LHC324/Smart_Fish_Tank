#line 1 "..\\GUI\\lvgl_src\\lv_widgets\\lv_win.c"
...
#line 10 "..\\GUI\\lvgl_src\\lv_widgets\\lv_win.c"


#line 13 "..\\GUI\\lvgl_src\\lv_widgets\\lv_win.c"
...
#line 14 "..\\GUI\\lvgl_src\\lv_widgets\\lv_win.c"
...
#line 15 "..\\GUI\\lvgl_src\\lv_widgets\\lv_win.c"
...
enum {
    LV_WIN_BTN_ALIGN_RIGHT = 0,      
    LV_WIN_BTN_ALIGN_LEFT    
};
...
lv_obj_t * lv_win_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    do { if(!((lv_debug_check_null(new_win)))) { ; lv_debug_log_error("Out of memory", (uint64_t)((uintptr_t)new_win)); while(1); } } while(0);;
    if(new_win == 0) return 0;

    if(ancestor_signal == 0) ancestor_signal = lv_obj_get_signal_cb(new_win);
    ...
    ext->page          = 0;
    ext->header        = 0;
    ext->title_txt    = lv_mem_alloc(strlen("Window") + 1);
    strcpy(ext->title_txt, "Window");
    ...
    if(copy == 0) {
        ...
        ext->btn_w = (65 == 0 ? 0 :(((( lv_disp_get_dpi(0) * (65) + 80) / 160)) > (1) ? ((( lv_disp_get_dpi(0) * (65) + 80) / 160)) : (1)));

        ext->page = lv_page_create(new_win, 0);
        ...
        ext->header = lv_obj_create(new_win, 0);
         
        lv_obj_add_protect(ext->header, LV_PROTECT_PARENT);
        lv_obj_set_parent(ext->header, new_win);
        if(ancestor_header_design == 0) ancestor_header_design = lv_obj_get_design_cb(ext->header);
        lv_obj_set_height(ext->header, (65 == 0 ? 0 :(((( lv_disp_get_dpi(0) * (65) + 80) / 160)) > (1) ? ((( lv_disp_get_dpi(0) * (65) + 80) / 160)) : (1))));
    ...
    lv_obj_refresh_style(new_win, LV_OBJ_PART_ALL, 0xFF);
...
void lv_win_clean(lv_obj_t * win)
{
    do { if(!((lv_debug_check_null(win)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)win)); while(1); } } while(0);;
...
{
    do { if(!((lv_debug_check_null(win)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)win)); while(1); } } while(0);;
    do { if(!((lv_debug_check_null(img_src)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)img_src)); while(1); } } while(0);;
...
{
    do { if(!((lv_debug_check_null(win)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)win)); while(1); } } while(0);;
    do { if(!((lv_debug_check_null(title)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)title)); while(1); } } while(0);;

    lv_win_ext_t * ext = lv_obj_get_ext_attr(win);

    ext->title_txt    = lv_mem_realloc(ext->title_txt, strlen(title) + 1);
    do { if(!((lv_debug_check_null(ext->title_txt)))) { ; lv_debug_log_error("Out of memory", (uint64_t)((uintptr_t)ext->title_txt)); while(1); } } while(0);;
    if(ext->title_txt == 0) return;
...
lv_obj_t * lv_win_get_from_btn(const lv_obj_t * ctrl_btn)
{
    do { if(!((lv_debug_check_null(ctrl_btn)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)ctrl_btn)); while(1); } } while(0);;
...
{
    do { if(!((lv_debug_check_null(win)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)win)); while(1); } } while(0);;
    do { if(!((lv_debug_check_null(obj)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)obj)); while(1); } } while(0);;
...
static lv_design_res_t lv_win_header_design(lv_obj_t * header, const lv_area_t * clip_area, lv_design_mode_t mode)
{
    ...
    if(mode == LV_DESIGN_COVER_CHK) {
        ...
        _lv_txt_get_size(&txt_size, ext->title_txt, label_dsc.font, label_dsc.letter_space, label_dsc.line_space, ((lv_coord_t)((uint32_t)((uint32_t)1 << (8 * sizeof(lv_coord_t) - 1)) - 1000)),
                         label_dsc.flag);
        ...
        btn = lv_obj_get_child_back(ext->header, 0);
        while(btn != 0) {
        ...
        lv_draw_label(&txt_area, clip_area, &label_dsc, ext->title_txt, 0);
    }
...
static lv_res_t lv_win_signal(lv_obj_t * win, lv_signal_t sign, void * param)
{
    ...
    if(sign == LV_SIGNAL_GET_TYPE) return lv_obj_handle_get_type_signal(param, "lv_win");
...
static void lv_win_realign(lv_obj_t * win)
{
    ...
    if(ext->page == 0 || ext->header == 0) return;

    lv_obj_set_width(ext->header, lv_obj_get_width(win));

    lv_obj_t * btn;
    lv_obj_t * btn_prev_at_left = 0;
    lv_obj_t * btn_prev_at_right = 0;

    _Bool is_header_right_side_empty = 1;
    _Bool is_header_left_side_empty = 1;
...
static lv_obj_t * lv_win_btn_create(lv_obj_t * par, const void * img_src)
{
    ...
    win_btn = lv_btn_create(par, 0);
    do { if(!((lv_debug_check_null(win_btn)))) { ; lv_debug_log_error("Out of memory", (uint64_t)((uintptr_t)win_btn)); while(1); } } while(0);;
    if(win_btn == 0) return 0;
    ...
    lv_obj_t * img = lv_img_create(win_btn, 0);
