#line 1 "..\\GUI\\lvgl_src\\lv_widgets\\lv_gauge.c"
...
#line 10 "..\\GUI\\lvgl_src\\lv_widgets\\lv_gauge.c"


#line 13 "..\\GUI\\lvgl_src\\lv_widgets\\lv_gauge.c"
#line 14 "..\\GUI\\lvgl_src\\lv_widgets\\lv_gauge.c"
...
#line 15 "..\\GUI\\lvgl_src\\lv_widgets\\lv_gauge.c"
#line 16 "..\\GUI\\lvgl_src\\lv_widgets\\lv_gauge.c"
#line 17 "..\\GUI\\lvgl_src\\lv_widgets\\lv_gauge.c"
...
#line 18 "..\\GUI\\lvgl_src\\lv_widgets\\lv_gauge.c"
...
#line 19 "..\\GUI\\lvgl_src\\lv_widgets\\lv_gauge.c"
...
#line 20 "..\\GUI\\lvgl_src\\lv_widgets\\lv_gauge.c"
#line 21 "..\\GUI\\lvgl_src\\lv_widgets\\lv_gauge.c"
...
lv_obj_t * lv_gauge_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    do { if(!((lv_debug_check_null(gauge)))) { ; lv_debug_log_error("Out of memory", (uint64_t)((uintptr_t)gauge)); while(1); } } while(0);;
    if(gauge == 0) return 0;
    ...
    if(ancestor_signal == 0) ancestor_signal = lv_obj_get_signal_cb(gauge);
    if(ancestor_design == 0) ancestor_design = lv_obj_get_design_cb(gauge);
    ...
    if(copy == 0) {
        lv_gauge_set_scale(gauge, 270, 21, 6);
        lv_gauge_set_needle_count(gauge, 1, 0);
        ...
        lv_obj_refresh_style(gauge, LV_OBJ_PART_ALL, 0xFF);
    }
...
void lv_gauge_set_needle_count(lv_obj_t * gauge, uint8_t needle_cnt, const lv_color_t colors[])
{
    do { if(!((lv_debug_check_null(gauge)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)gauge)); while(1); } } while(0);;
    ...
    if(ext->needle_count != needle_cnt) {
        ...
        do { if(!((lv_debug_check_null(ext->values)))) { ; lv_debug_log_error("Out of memory", (uint64_t)((uintptr_t)ext->values)); while(1); } } while(0);;
        if(ext->values == 0) return;
...
void lv_gauge_set_value(lv_obj_t * gauge, uint8_t needle_id, int32_t value)
{
    ...
    p_end.y = (_lv_trigo_sin(needle_angle) * r) / 32767 + y_ofs;
    p_end.x = (_lv_trigo_sin(needle_angle + 90) * r) / 32767 + x_ofs;

    lv_area_t a;
    a.x1 = ((p_mid . x) < (p_end . x) ? (p_mid . x) : (p_end . x)) - needle_w;
    a.y1 = ((p_mid . y) < (p_end . y) ? (p_mid . y) : (p_end . y)) - needle_w;
    a.x2 = ((p_mid . x) > (p_end . x) ? (p_mid . x) : (p_end . x)) + needle_w;
    a.y2 = ((p_mid . y) > (p_end . y) ? (p_mid . y) : (p_end . y)) + needle_w;
...
static lv_res_t lv_gauge_signal(lv_obj_t * gauge, lv_signal_t sign, void * param)
{
    ...
    if(sign == LV_SIGNAL_GET_TYPE) return lv_obj_handle_get_type_signal(param, "lv_gauge");
...
static void lv_gauge_draw_labels(lv_obj_t * gauge, const lv_area_t * mask)
{
    ...
    for(i = 0; i < label_num; i++) {
        ...
        lv_coord_t y = (int32_t)((int32_t)_lv_trigo_sin(angle) * r) / 32767;
        y += y_ofs;

        lv_coord_t x = (int32_t)((int32_t)_lv_trigo_sin(angle + 90) * r) / 32767;
        x += x_ofs;
        ...
        lv_draw_label(&label_cord, mask, &label_dsc, scale_txt, 0);
    }
}
...
static void lv_gauge_draw_needle(lv_obj_t * gauge, const lv_area_t * clip_area)
{
    ...
    for(i = 0; i < ext->needle_count; i++) {
        ...
        if(ext->needle_img == 0) {
            ...
            if(ext->needle_colors != 0) line_dsc.color = ext->needle_colors[i];
