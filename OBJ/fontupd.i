...
u32 fupd_prog(u16 x,u16 y,u8 size,u32 fsize,u32 pos)
{
	...
	if(t!=prog)
	{
		LCD_ShowString(x+3*size/2,y,128,12,size,"%");		
		t=prog;
		if(t>100)t=100;
...
u8 updata_fontx(u16 x,u16 y,u8 size,u8 *fxpath,u8 fx)
{
	...
	u32 offx=0;
	u8 rval=0;	     
	fftemp=(FIL*)mymalloc(0,sizeof(FIL));	
	if(fftemp==0)rval=1;
	tempbuf=mymalloc(0,4096);				
	if(tempbuf==0)rval=1;
 	res=f_open(fftemp,(const TCHAR*)fxpath,0x01); 
 	if(res)rval=2;
 	if(rval==0)	 
	{
		switch(fx)
		{
			case 0:												
				ftinfo.ugbkaddr=1024*1024*4+sizeof(ftinfo);	
			...
			case 3:
				...
				ftinfo.gkb24size=fftemp->obj.objsize;					
...
{	
	...
	strcat((char*)pname,(char*)UNIGBK_PATH); 
 	res=f_open(fftemp,(const TCHAR*)pname,0x01); 
 	if(res)rval|=1<<4;
	strcpy((char*)pname,(char*)src);	
	...
	if(rval==0)
	{  
		LCD_ShowString(x,y,128,12,size,"sectors...");
		for(i=0;i<791;i++)	
		{
			fupd_prog(x+20*size/4,y,size,791,i);
			W25QXX_Read((u8*)buf,((1024*1024*4/4096)+i)*4096,4096);
			for(j=0;j<1024;j++)
			{
				if(buf[j]!=0XFFFFFFFF)break;
			}
			if(j!=1024)W25QXX_Erase_Sector((1024*1024*4/4096)+i);	
		}
		myfree(0,buf);
		LCD_ShowString(x,y,128,12,size,"UNIGBK.BIN");		
		strcpy((char*)pname,(char*)src);				
		strcat((char*)pname,(char*)UNIGBK_PATH); 
		res=updata_fontx(x+20*size/4,y,size,pname,0);	
		if(res){myfree(0,pname);return 1;}
		LCD_ShowString(x,y,128,12,size,"GBK12.BIN  ");
		strcpy((char*)pname,(char*)src);				
		strcat((char*)pname,(char*)GBK12_PATH); 
		res=updata_fontx(x+20*size/4,y,size,pname,1);	
		if(res){myfree(0,pname);return 2;}
		LCD_ShowString(x,y,128,12,size,"GBK16.BIN  ");
		strcpy((char*)pname,(char*)src);				
		strcat((char*)pname,(char*)GBK16_PATH); 
		res=updata_fontx(x+20*size/4,y,size,pname,2);	
		if(res){myfree(0,pname);return 3;}
		LCD_ShowString(x,y,128,12,size,"GBK24.BIN  ");
		strcpy((char*)pname,(char*)src);				
		strcat((char*)pname,(char*)GBK24_PATH); 
		res=updata_fontx(x+20*size/4,y,size,pname,3);	
		if(res){myfree(0,pname);return 4;}
		
		ftinfo.fontok=0XAA;
		W25QXX_Write((u8*)&ftinfo,1024*1024*4,sizeof(ftinfo));	
	}
	myfree(0,pname);
	myfree(0,buf);
	return rval;
...
{		
	...
	{
		t++;
		W25QXX_Read((u8*)&ftinfo,1024*1024*4,sizeof(ftinfo));
