#line 1 "..\\GUI\\lvgl_src\\lv_widgets\\lv_objmask.c"
...
#line 10 "..\\GUI\\lvgl_src\\lv_widgets\\lv_objmask.c"
#line 11 "..\\GUI\\lvgl_src\\lv_widgets\\lv_objmask.c"
...
#line 12 "..\\GUI\\lvgl_src\\lv_widgets\\lv_objmask.c"
...
#line 13 "..\\GUI\\lvgl_src\\lv_widgets\\lv_objmask.c"
...
lv_obj_t * lv_objmask_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    do { if(!((lv_debug_check_null(objmask)))) { ; lv_debug_log_error("Out of memory", (uint64_t)((uintptr_t)objmask)); while(1); } } while(0);;
    if(objmask == 0) return 0;
    ...
    if(ancestor_signal == 0) ancestor_signal = lv_obj_get_signal_cb(objmask);
    if(ancestor_design == 0) ancestor_design = lv_obj_get_design_cb(objmask);
    ...
    if(copy == 0) {
        ...
        lv_obj_refresh_style(objmask, LV_OBJ_PART_ALL, 0xFF);
    }
...
lv_objmask_mask_t * lv_objmask_add_mask(lv_obj_t * objmask, void * param)
{
    do { if(!((lv_debug_check_null(objmask)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)objmask)); while(1); } } while(0);;
    do { if(!((lv_debug_check_null(param)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)param)); while(1); } } while(0);;
    ...
    do { if(!((lv_debug_check_null(m)))) { ; lv_debug_log_error("Out of memory", (uint64_t)((uintptr_t)m)); while(1); } } while(0);;
    if(m == 0) return 0;
    m->param = lv_mem_alloc(param_size);
    do { if(!((lv_debug_check_null(m->param)))) { ; lv_debug_log_error("Out of memory", (uint64_t)((uintptr_t)m->param)); while(1); } } while(0);;
    if(m->param == 0) return 0;
...
{
    do { if(!((lv_debug_check_null(objmask)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)objmask)); while(1); } } while(0);;
    do { if(!((lv_debug_check_null(mask)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)mask)); while(1); } } while(0);;
    do { if(!((lv_debug_check_null(param)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)param)); while(1); } } while(0);;
    lv_draw_mask_common_dsc_t * dsc = param;
...
void lv_objmask_remove_mask(lv_obj_t * objmask, lv_objmask_mask_t * mask)
{
    ...
    if(mask == 0) {
        lv_objmask_mask_t * m;
        for(m = _lv_ll_get_head(&ext->mask_ll); m != 0; m = _lv_ll_get_next(&ext->mask_ll, m)) {
            lv_mem_free(m->param);
        }
...
static lv_res_t lv_objmask_signal(lv_obj_t * objmask, lv_signal_t sign, void * param)
{
    ...
    if(sign == LV_SIGNAL_GET_TYPE) return lv_obj_handle_get_type_signal(param, "lv_objmask");

    if(sign == LV_SIGNAL_CLEANUP) {
        lv_objmask_ext_t * ext = lv_obj_get_ext_attr(objmask);
        lv_objmask_mask_t * i;
        for(i = _lv_ll_get_head(&ext->mask_ll); i != 0; i = _lv_ll_get_next(&ext->mask_ll, i)) {
            if(i->param) {
                lv_mem_free(i->param);
                i->param = 0;
            }
