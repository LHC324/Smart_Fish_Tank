#line 1 "..\\GUI\\lvgl_src\\lv_draw\\lv_img_cache.c"
...
#line 10 "..\\GUI\\lvgl_src\\lv_draw\\lv_img_cache.c"
...
#line 11 "..\\GUI\\lvgl_src\\lv_draw\\lv_img_cache.c"
#line 12 "..\\GUI\\lvgl_src\\lv_draw\\lv_img_cache.c"
...
#line 13 "..\\GUI\\lvgl_src\\lv_draw\\lv_img_cache.c"
...
#line 14 "..\\GUI\\lvgl_src\\lv_draw\\lv_img_cache.c"
...
#line 15 "..\\GUI\\lvgl_src\\lv_draw\\lv_img_cache.c"
...
lv_img_cache_entry_t * _lv_img_cache_open(const void * src, lv_color_t color)
{
    ...
    lv_img_cache_entry_t * cache = _lv_img_cache_array;

     
    uint16_t i;
    for(i = 0; i < entry_cnt; i++) {
        if(cache[i].life > (~0x7fffffff) + 1) {
            cache[i].life -= 1;
        }
    }

     
    lv_img_cache_entry_t * cached_src = 0;
    for(i = 0; i < entry_cnt; i++) {
        _Bool match = 0;
        lv_img_src_t src_type = lv_img_src_get_type(cache[i].dec_dsc.src);
        if(src_type == LV_IMG_SRC_VARIABLE) {
            if(cache[i].dec_dsc.src == src && cache[i].dec_dsc.color.full == color.full) match = 1;
        }
        else if(src_type == LV_IMG_SRC_FILE) {
            if(strcmp(cache[i].dec_dsc.src, src) == 0) match = 1;
        }
        ...
        if(match) {
            ...
            cached_src = &cache[i];
            cached_src->life += cached_src->dec_dsc.time_to_open * 1;
            if(cached_src->life > 1000) cached_src->life = 1000;
            ;
            break;
        }
    }
...
{
    if(_lv_img_cache_array != 0) {
         
        lv_img_cache_invalidate_src(0);
        lv_mem_free(_lv_img_cache_array);
    }

     
    _lv_img_cache_array = lv_mem_alloc(sizeof(lv_img_cache_entry_t) * new_entry_cnt);
    do { if(!((lv_debug_check_null(_lv_img_cache_array)))) { ; lv_debug_log_error("Out of memory", (uint64_t)((uintptr_t)_lv_img_cache_array)); while(1); } } while(0);;
    if(_lv_img_cache_array == 0) {
        entry_cnt = 0;
        return;
    }
    ...
    uint16_t i;
    for(i = 0; i < entry_cnt; i++) {
        _lv_memset_00(&_lv_img_cache_array[i].dec_dsc, sizeof(lv_img_decoder_dsc_t));
        _lv_memset_00(&_lv_img_cache_array[i], sizeof(lv_img_cache_entry_t));
    }
}
...
void lv_img_cache_invalidate_src(const void * src)
{
    ...
    uint16_t i;
    for(i = 0; i < entry_cnt; i++) {
        if(cache[i].dec_dsc.src == src || src == 0) {
            if(cache[i].dec_dsc.src != 0) {
