#line 1 "..\\GUI\\lvgl_src\\lv_widgets\\lv_linemeter.c"
...
#line 10 "..\\GUI\\lvgl_src\\lv_widgets\\lv_linemeter.c"


#line 13 "..\\GUI\\lvgl_src\\lv_widgets\\lv_linemeter.c"
...
#line 14 "..\\GUI\\lvgl_src\\lv_widgets\\lv_linemeter.c"
...
#line 15 "..\\GUI\\lvgl_src\\lv_widgets\\lv_linemeter.c"
...
#line 16 "..\\GUI\\lvgl_src\\lv_widgets\\lv_linemeter.c"
#line 17 "..\\GUI\\lvgl_src\\lv_widgets\\lv_linemeter.c"
...
lv_obj_t * lv_linemeter_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    do { if(!((lv_debug_check_null(linemeter)))) { ; lv_debug_log_error("Out of memory", (uint64_t)((uintptr_t)linemeter)); while(1); } } while(0);;
    if(linemeter == 0) return 0;

    if(ancestor_signal == 0) ancestor_signal = lv_obj_get_signal_cb(linemeter);
    ...
    if(copy == 0) {
        lv_obj_set_size(linemeter, 3 * 60 / 2, 3 * 60 / 2);
        ...
        lv_obj_refresh_style(linemeter, LV_OBJ_PART_ALL, 0xFF);
    }
...
void lv_linemeter_set_value(lv_obj_t * lmeter, int32_t value)
{
    do { if(!((lv_debug_check_null(lmeter)))) { ; lv_debug_log_error("NULL pointer", (uint64_t)((uintptr_t)lmeter)); while(1); } } while(0);;
    ...
    line_width = ((line_width) > (end_line_width) ? (line_width) : (end_line_width));

    int32_t angle_old = (level_old * ext->scale_angle) / (ext->line_cnt - 1);

     
    int32_t y_in_old  = (int32_t)((int32_t)_lv_trigo_sin(angle_old + angle_ofs) * r_in) >> 15;
    int32_t x_in_old  = (int32_t)((int32_t)_lv_trigo_sin(angle_old + 90 + angle_ofs) * r_in) >> 15;


    int32_t y_out_old  = (int32_t)((int32_t)_lv_trigo_sin(angle_old + angle_ofs) * r_out) >> 15;
    int32_t x_out_old  = (int32_t)((int32_t)_lv_trigo_sin(angle_old + 90 + angle_ofs) * r_out) >> 15;
    ...
    int32_t y_in_new  = (int32_t)((int32_t)_lv_trigo_sin(angle_new + angle_ofs) * r_in) >> 15;
    int32_t x_in_new  = (int32_t)((int32_t)_lv_trigo_sin(angle_new + 90 + angle_ofs) * r_in) >> 15;


    int32_t y_out_new  = (int32_t)((int32_t)_lv_trigo_sin(angle_new + angle_ofs) * r_out) >> 15;
    int32_t x_out_new  = (int32_t)((int32_t)_lv_trigo_sin(angle_new + 90 + angle_ofs) * r_out) >> 15;

    lv_area_t a;
    if(x_out_old < 0 && x_out_new < 0) {
        a.x1 = lmeter->coords.x1 + left - line_width;
        a.y1 = (((((y_out_old) < (y_out_new) ? (y_out_old) : (y_out_new))) < (((y_in_old) < (y_in_new) ? (y_in_old) : (y_in_new))) ? (((y_out_old) < (y_out_new) ? (y_out_old) : (y_out_new))) : (((y_in_old) < (y_in_new) ? (y_in_old) : (y_in_new))))) + y_ofs - line_width;
        a.x2 = ((x_in_old) > (x_in_new) ? (x_in_old) : (x_in_new)) + x_ofs + line_width;
        a.y2 = (((((y_out_old) > (y_out_new) ? (y_out_old) : (y_out_new))) > (((y_in_old) > (y_in_new) ? (y_in_old) : (y_in_new))) ? (((y_out_old) > (y_out_new) ? (y_out_old) : (y_out_new))) : (((y_in_old) > (y_in_new) ? (y_in_old) : (y_in_new))))) + y_ofs + line_width;
    }
    else if(x_out_old > 0 && x_out_new > 0) {
        a.x1 = ((x_in_old) < (x_in_new) ? (x_in_old) : (x_in_new)) + x_ofs - line_width;
        a.y1 = (((((y_out_old) < (y_out_new) ? (y_out_old) : (y_out_new))) < (((y_in_old) < (y_in_new) ? (y_in_old) : (y_in_new))) ? (((y_out_old) < (y_out_new) ? (y_out_old) : (y_out_new))) : (((y_in_old) < (y_in_new) ? (y_in_old) : (y_in_new))))) + y_ofs - line_width;
        a.x2 = lmeter->coords.x2 - right + line_width;
        a.y2 = (((((y_out_old) > (y_out_new) ? (y_out_old) : (y_out_new))) > (((y_in_old) > (y_in_new) ? (y_in_old) : (y_in_new))) ? (((y_out_old) > (y_out_new) ? (y_out_old) : (y_out_new))) : (((y_in_old) > (y_in_new) ? (y_in_old) : (y_in_new))))) + y_ofs + line_width;
    }
    else if(y_out_old < 0 && y_out_new < 0) {
        a.x1 = (((((x_out_old) < (x_out_new) ? (x_out_old) : (x_out_new))) < (((x_in_old) < (x_in_new) ? (x_in_old) : (x_in_new))) ? (((x_out_old) < (x_out_new) ? (x_out_old) : (x_out_new))) : (((x_in_old) < (x_in_new) ? (x_in_old) : (x_in_new))))) + x_ofs - line_width;
        a.y1 = lmeter->coords.y1 + top - line_width;
        a.x2 = (((((x_out_old) > (x_out_new) ? (x_out_old) : (x_out_new))) > (((x_in_old) > (x_in_new) ? (x_in_old) : (x_in_new))) ? (((x_out_old) > (x_out_new) ? (x_out_old) : (x_out_new))) : (((x_in_old) > (x_in_new) ? (x_in_old) : (x_in_new))))) + x_ofs + line_width;
        a.y2 = ((y_in_old) > (y_in_new) ? (y_in_old) : (y_in_new)) + y_ofs + line_width;
    }
    else if(y_out_old > 0 && y_out_new > 0) {
        a.x1 = (((((x_out_old) < (x_out_new) ? (x_out_old) : (x_out_new))) < (((x_in_old) < (x_in_new) ? (x_in_old) : (x_in_new))) ? (((x_out_old) < (x_out_new) ? (x_out_old) : (x_out_new))) : (((x_in_old) < (x_in_new) ? (x_in_old) : (x_in_new))))) + x_ofs - line_width;
        a.y1 = ((y_in_old) < (y_in_new) ? (y_in_old) : (y_in_new)) + y_ofs - line_width;
        a.x2 = (((((x_out_old) > (x_out_new) ? (x_out_old) : (x_out_new))) > (((x_in_old) > (x_in_new) ? (x_in_old) : (x_in_new))) ? (((x_out_old) > (x_out_new) ? (x_out_old) : (x_out_new))) : (((x_in_old) > (x_in_new) ? (x_in_old) : (x_in_new))))) + x_ofs + line_width;
...
void lv_linemeter_draw_scale(lv_obj_t * lmeter, const lv_area_t * clip_area, uint8_t part)
{
    ...
    lv_draw_mask_radius_init(&mask_in_param, &mask_area, (0x7FFF), 1);
    int16_t mask_in_id = lv_draw_mask_add(&mask_in_param, 0);



#line 438 "..\\GUI\\lvgl_src\\lv_widgets\\lv_linemeter.c"
    ...
    for(i = 0; i < ext->line_cnt; i++) {
        ...
        int32_t y_out_extra = (int32_t)((int32_t)sin_mid * r_out_extra) >> (15 - 8);
        int32_t x_out_extra = (int32_t)((int32_t)cos_mid * r_out_extra) >> (15 - 8);
...
#line 489 "..\\GUI\\lvgl_src\\lv_widgets\\lv_linemeter.c"
        int32_t x_in_extra = x_ofs;
        int32_t y_in_extra = y_ofs;


         
        int32_t y_in  = (int32_t)((int32_t)_lv_trigo_sin(angle_normal + angle_ofs) * r_in) >> 15;
        int32_t x_in  = (int32_t)((int32_t)_lv_trigo_sin(angle_normal + 90 + angle_ofs) * r_in) >> 15;

        x_in += x_ofs;
        y_in += y_ofs;

        int32_t y_out  = (int32_t)((int32_t)_lv_trigo_sin(angle_normal + angle_ofs) * r_out) >> 15;
        int32_t x_out  = (int32_t)((int32_t)_lv_trigo_sin(angle_normal + 90 + angle_ofs) * r_out) >> 15;

        x_out += x_ofs;
        y_out += y_ofs;

        lv_area_t clip_sub;
        clip_sub.x1 = ((x_in) < (x_out) ? (x_in) : (x_out)) - line_dsc.width;
        clip_sub.x2 = ((x_in) > (x_out) ? (x_in) : (x_out)) + line_dsc.width;
        clip_sub.y1 = ((y_in) < (y_out) ? (y_in) : (y_out)) - line_dsc.width;
        clip_sub.y2 = ((y_in) > (y_out) ? (y_in) : (y_out)) + line_dsc.width;

        if(_lv_area_intersect(&clip_sub, &clip_sub, clip_area) == 0) continue;
...
static lv_res_t lv_linemeter_signal(lv_obj_t * lmeter, lv_signal_t sign, void * param)
{
    ...
    if(sign == LV_SIGNAL_GET_TYPE) return lv_obj_handle_get_type_signal(param, "lv_linemeter");
