...
//		__HAL_RCC_GPIOF_CLK_ENABLE();			//开启GPIOF时钟
		...
		//GPIOB14初始化设置:MISO
		GPIO_Initure.Pin=GPIO_PIN_14;	//PB14 设置为上拉输入
		GPIO_Initure.Mode=GPIO_MODE_INPUT;  	//输入模式
		GPIO_Initure.Pull=GPIO_PULLUP;          //上拉
		GPIO_Initure.Speed=GPIO_SPEED_HIGH;     //高速
		HAL_GPIO_Init(GPIOB,&GPIO_Initure);     //初始化
        
		//PB12、13、15：CS/SCK/MOSI
		GPIO_Initure.Pin=GPIO_PIN_12|GPIO_PIN_13|GPIO_PIN_15; //PB0设置为推挽输出
		GPIO_Initure.Mode=GPIO_MODE_OUTPUT_PP;  //推挽输出
		HAL_GPIO_Init(GPIOB,&GPIO_Initure);     //初始化
        
		//PC5:PEN
		GPIO_Initure.Pin=GPIO_PIN_5;          	//PC5设置为推挽输出
		GPIO_Initure.Mode=GPIO_MODE_INPUT;  	//输入模式
		GPIO_Initure.Pull=GPIO_PULLUP;          //上拉
		GPIO_Initure.Speed=GPIO_SPEED_HIGH;     //高速
